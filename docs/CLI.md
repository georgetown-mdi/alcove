...
Exchange details are stored in two files: a configuration file and an authentication key file. The default file names and paths are `./alcove.yaml` and `./.alcove.key`, while command line arguments to override are `--config-file` and `--key-file` respectively. When these files are first created, the application prints a notice identifying both and gives a warning that the key file should be treated as private. For Docker deployments, agencies are expected to mount one directory per exchange partner, so the working directory itself provides isolation and no subdirectory is needed. A configuration and key file written by a build from before the product was renamed Alcove still have the earlier product's name, which no command looks for; rename them to `alcove.yaml` and `.alcove.key` by hand.
...
- **What cannot introduce it.** An invitation holds no credential by construction, and the connection details on the accept path come from your own command line, so only a wholesale substituted configuration file reaches this.
...
**Where it is required.** [`alcove invite`](#offline-invitation) and [`alcove accept`](#offline-acceptance) author a durable partnership -- a configuration, an invitation, the instructions a partner follows -- so each refuses to proceed unnamed and exits 64.

**Where a configuration supplies it.** A run over a pre-existing configuration that it does not rewrite -- inviting from one, or [accepting over one it keeps](#existing-files) -- takes the label from that file's `linkage_terms.identity` and reports a typed `--identity` as having no effect. A configuration holding none is refused, naming the field to set, and so is one still holding the placeholder below: the file (not this one invocation) is what every later run sends, so renaming the party is an edit of that file.

**Where it is optional.** The zero-setup form runs with or without it: given a label, that label rides into the terms; given none, the terms hold none, and nothing is asked. [`alcove exchange`](#recurring-exchange) takes the configuration's `linkage_terms.identity`, which `--identity` replaces for that one run, and runs unnamed when neither supplies a label.

**Where it is asked for.** With a terminal on standard input and no `--identity`, [`alcove init`](#initialization) and [`alcove accept`](#offline-acceptance) ask for the label rather than leaving you to the flag, and the answer goes into the configuration file that command was already going to write -- nowhere else, and nothing is kept outside the directory you pointed the command at.

- **The flag is the non-interactive path on both**: supplying it asks nothing, and a run with no terminal to ask at -- a pipe, a container started without `-t`, a CI job -- asks nothing either.
- **What that run does instead is the command's own rule**: `init` writes the placeholder below, and `accept` refuses (exit 64) rather than blocking on a question nothing will answer.
- **An answer takes exactly the treatment a flag value takes**: trimmed, blank read as absence, and the template placeholder refused.
- **Nothing is asked where a command writes no configuration** -- an acceptance that keeps an existing one, an `init` whose overwrite you declined -- since there is no file for the answer to be remembered in.
- **`alcove invite`, `alcove exchange`, and the zero-setup form never ask**: their label comes from a configuration file they do not rewrite, or from the flag alone.

**What absence means.** Alcove stands in no label of its own -- not the account it runs as, not a placeholder. A run with no identity sends none: the terms omit the field, the exchange record omits its own, and every surface that shows a party name shows an absence marker (`(no name given)`) rather than inventing one. A signed receipt is the one thing an unnamed run cannot produce: a certificate is trusted by the identity its holder used in the agreed terms, so a run with `signing.mode: certificate` and an unnamed party on either side stops rather than producing a receipt nobody can verify -- for this party's own absence before the exchange starts (exit 64, see [Signing identity and the agreed terms](#signing-identity-and-the-agreed-terms)), and for the partner's at terms agreement, the first point their terms are in hand, so the run ends before any linkage key or data crosses.

A flag value that is empty or blank -- what a scripted `--identity "$ORG"` sends when `ORG` is unset -- is treated as absence, not as a label, and a blank `linkage_terms.identity` in a configuration file is treated the same way where a label is required:

- Where the flag is required, an empty or blank value is a usage error (exit 64); where it is optional, the run proceeds with no identity.
- `alcove invite` over an existing configuration refuses a whitespace-only `linkage_terms.identity` exactly as it refuses a missing one, rather than minting an invitation whose inviter heading renders empty, and an acceptance that keeps one refuses it the same way.
- The label a named configuration sends is its own bytes, whitespace included -- `alcove exchange` reads it straight from the file -- so for that path trimming decides only whether a run is refused, never what rides into the agreed terms.
- A `--identity` flag value is trimmed before it rides: what reaches the terms is the trimmed string, not the bytes typed at the flag.
- Either way a certificate authorizes an exact identity string, so a signing identity bound to a padded value diverges from a flag-supplied identity, which trims the padding away; an exchange that signs receipts under it is refused before it runs (see [Signing identity and the agreed terms](#signing-identity-and-the-agreed-terms)).

**What the template leaves you to fill in.** `alcove init` writes a placeholder into `linkage_terms.identity` when the run gives it no `--identity` -- like the connection's host and username, the template is a scaffold to hand-edit. That exact placeholder value is refused wherever a label is read, as firmly as an absent one: [`alcove invite`](#offline-invitation) over a configuration still holding it names the field to edit rather than minting an invitation under it, and on a path where the flag itself supplies the label, a command given it as `--identity` exits 64 before anything is sent. Over a [kept configuration](#existing-files) that already holds a real label, [`accept`](#offline-acceptance) -- like `invite` before it -- proceeds under that stored label and reports the placeholder flag as having no effect rather than exiting; the kept configuration's own placeholder is still refused (see [Offline acceptance](#offline-acceptance)). It is matched whole, ignoring surrounding whitespace, so a name that merely contains those words is a label like any other.
...
- `alcove exchange` expands `--config-file`, `--key-file`, `--record-file`, the input and output paths, and `signing.identity_file`.
...
The row check applies to every command that reads a CSV to set up or run an exchange (`alcove exchange`, the zero-setup form, `alcove invite`, `alcove accept`) and to the files `alcove verify-receipt` reads. The empty-dataset check applies to the exchange commands only: a result CSV with no rows is what a genuine zero-match exchange writes, and `alcove verify-receipt` must still be able to verify one. `alcove init` applies neither -- it reads the header and a bounded sample of one column to write its template, never the whole file -- so a template can still be authored from a file these checks would refuse for an exchange.

### The field delimiter

`--csv-delimiter` sets the field delimiter of the CSV a command reads and of the result file it writes, for a source system that exports pipe-, tab-, or semicolon-separated data. The default is a comma: a run that names no delimiter reads commas and writes commas. The configuration field is [`csv_delimiter`](EXCHANGE_REFERENCE.md#csv_delimiter); the flag replaces it for one run.

- **Accepted values.** One character: a tab, or a printable ASCII character other than the double quote (`"`), which is the character RFC 4180 quotes a field with. Also `detect`, the one value that is not a character. Anything else -- more than one character, an empty value, the double quote, a line break, a character outside ASCII -- is refused with a usage error (exit 64) before any credential, terms, or data are sent.
- **Writing a tab.** On the command line and in the configuration file, write a tab as `tab` or `\t`: `alcove exchange --csv-delimiter tab data.tsv results.tsv`. A literal tab is accepted where you can type one.
- **The result follows the delimiter you name.** There is no separate output setting: a run that reads pipes writes pipes, so the result file can be fed straight back to the system the input came from. A run that names `detect`, or names nothing, writes commas. A field holding the delimiter, a double quote, or a line break is quoted on the way out, so the result reads back through the same delimiter.
...
- **Where it applies.** Every command that reads a CSV takes the flag: `alcove exchange`, the zero-setup form, `alcove invite`, `alcove accept`, `alcove init`, and `alcove verify-receipt`. A command that reads none does not accept it and reports it as an unknown option.
- **What `alcove init` does with it.** The template it writes records the delimiter it was given as `csv_delimiter`, so the recurring `alcove exchange` that template governs needs no flag. A zero-setup `--save` run and the configurations `invite`/`accept` write record it the same way.
- **When a configuration also sets one.** The flag governs the run you give it on, and the configuration's `csv_delimiter` governs a run given no flag. That is the rule on every command: for `alcove invite` and `alcove accept` the configuration is the one at the config path, for `alcove exchange` the one it runs from, and for `alcove verify-receipt` the one passed as `--config-file`, so the files an exchange wrote are re-read the way it wrote them.
- **Changing it for one run.** Where that configuration also governs later runs -- `alcove invite` taking its terms from an existing configuration, `alcove accept` keeping the configuration already at the config path, and `alcove exchange` -- a flag naming a different delimiter is reported: the run you gave it on uses the flag's delimiter, and every later exchange over that file reads and writes by the `csv_delimiter` the file records. Edit `csv_delimiter` there to change those; a flag is never written into a configuration Alcove only read.
- **Leaving it out.** Given no delimiter, Alcove reads the file with commas and writes the result with commas.
...
- **Duration format.** It is duration-valued like the flags above -- the unit suffix is still required, and a bare number is still rejected the same way -- but it additionally accepts a millisecond unit, so a sub-second value such as `100ms` is expressible; the millisecond unit is unique to this flag, and the other duration options still reject a sub-second or `ms` value.
- **What the interval costs.** Every wait for the partner's next file costs up to one interval, and an exchange makes many, so the interval adds up across a run. What the PSI masking beside it costs is in [PROTOCOL.md](spec/PROTOCOL.md), "Masking compute is a wall-clock expectation". The conservative default is what stays within SFTP servers' anti-flood limits, so lower it where you control the directory or the server -- a local mount, CI, or a demo -- and leave it as it is against a partner's shared server.
- **The floor.** There is no hard floor, but a value below `1s` is warned about (not blocked): a sub-second poll can trip an SFTP server's anti-flood/DoS protection and drop the connection.
- **Where it applies.** The flag takes effect on the commands that build a live connection -- the zero-setup exchange, `alcove exchange`, and the online `invite`/`accept` -- and, like the other connection-tuning flags, is reported as ignored on an offline `invite`/`accept` (set `poll_interval_ms` under `connection.options` in the written configuration instead). The one exception in that class is `--peer-timeout`, which an [acceptance that runs the exchange itself](#accepting-and-running-a-webrtc-exchange) applies rather than reports.
...
`linkage_terms.linkage_rule_set` names the rule set your linkage fields and keys were drawn from, and every path that fills those lists in for you writes it -- including the template `alcove init` produces. Nothing re-decides it when you edit `linkage_fields` or `linkage_keys` by hand, so a citation can outlive the rules it describes and travel that way onto the invitation, your partner's terms review, and both parties' exchange records. See [`linkage_terms.linkage_rule_set`](EXCHANGE_REFERENCE.md#linkage_termslinkage_rule_set) for what the citation does and does not decide.

Every command that reads linkage terms out of a configuration file checks the citation against that file's own rules and warns when they no longer support it, naming the cited set and which of the two lists diverged: `alcove exchange`, an offline `alcove invite` that takes its terms from the configuration, an `alcove accept` that reuses one already at the path, and `alcove verify-receipt` reading your terms from `--config-file`. It is a warning, not a refusal -- the file is yours, and the citation is recorded and displayed but never selects or alters matching, so the command proceeds. Omit `linkage_rule_set` for rules you author yourself, or restore the rules the cited set declares.

That warning is an ordinary diagnostic, so a raised `--log-level` drops it. An exchange you authored yourself states the same finding again on the display it prints before the run ([What the run shows before it starts](#what-the-run-shows-before-it-starts)), which prints at every level -- so a scheduled run quieted to `error` still shows the divergence beside the keys it is about.

A configuration an acceptance stands behind is the other case, and the warning offers a different remedy there. Those terms are the ones you and the inviting party agreed on, so restoring the cited set's rules would edit that agreement single-handedly, and the exchange would refuse the result against the partner still running the originals. Settle the citation with that party and accept again, or decline to reuse those terms -- and on an offline `alcove invite`, where you are minting a new invitation from those terms rather than putting them to use, author fresh terms for the invitation instead.

That case is recognized from a record `alcove accept` writes into the configuration it produces or reuses, so only the accepting side of an exchange has it. An invitation you sent and a partner accepted leaves nothing in your own file to read, so your copy is still treated as terms you hold alone -- and the remedy offered there, restoring the cited set's rules, can still take terms out of an agreement the CLI has no way to see. Check with your partner before acting on it.

Only a set this build ships can be checked. A citation naming another set, or the built-in set at a version this build does not include, passes without comment: there is nothing behind that name here to compare your rules against. The field set and the key set are checked separately, so a citation pairing a name this build does not know with the built-in key set is still held to the built-in keys. A partner's citation on an invitation is their statement about their own rules; `alcove accept` runs this same check over it against the rules the invitation declares and shows the per-half verdict on the accept prompt before you consent.

A configuration file can name its rule set instead of writing the rules out: a `linkage_terms` block holding `linkage_rule_set` and neither `linkage_fields` nor `linkage_keys` runs on that set's rules, filled in as Alcove ships them. Nothing in such a file can diverge, so the check above has nothing to report on it -- a name this build does not ship stops the load instead, naming what the file cited and what this build ships. See [Naming a set instead of writing the rules out](EXCHANGE_REFERENCE.md#naming-a-set-instead-of-writing-the-rules-out).
...
`--deduplicate` lets several of THIS party's records match a single one of the partner's, the [`linkage_terms.deduplicate`](EXCHANGE_REFERENCE.md#linkage_termsdeduplicate) a configured exchange declares. It is off by default, and it is each party's own: unlike the strategy the two values need not agree, neither party reads the other's from its own command line, and the pair decides the exchange's cardinality once the terms are exchanged. Setting it widens what the partner discloses -- more of its records can match than in a one-to-one run of the same two files. Both parties setting it resolves to a many-to-many match, where each party's records may group the other's; that pair runs under either linkage strategy, as a one-sided pair does. With `--save`, the value is written into the saved configuration, so a later recurring `alcove exchange` runs the setting that was prototyped.
...
If `--save` is not specified, after running users are instructed how to use `alcove invite` and `alcove accept` to establish a recurring exchange. `--save` usage can be discussed during onboarding.
...
If a zero-setup exchange is started with configuration and/or key files already present, the user is warned that they will be ignored and that if their intent was to use those files, the user should use `alcove exchange` instead.
...
An invitation MAY include a connection endpoint: a public locator that tells the acceptor where to rendezvous (a PeerJS signaling URL, an SFTP host and port, or a file-drop directory) so the parties need not arrange that detail over a separate channel. A WebRTC endpoint may also name the inviter's relay by its TURN and STUN urls, which the acceptance shows before you consent and writes as [`invitation_relay`](EXCHANGE_REFERENCE.md#connectioninvitation_relay). The endpoint is the locator only and never includes credentials -- no password, private key, key file, PeerJS API key, or relay credential. Each party still supplies the credential portion of its own `connection` block independently. When an invitation omits the endpoint, both parties configure their `connection` block entirely on their own.
...
```sh
alcove invite --identity IDENTITY [--expires-in DURATION] [--linkage-strategy STRATEGY] [INPUT_FILE]
```
...
That authoring-time check grades the input against the configuration's linkage terms on the same verdict `alcove exchange` enforces at the run boundary, so an invitation is minted only when every agreed linkage key can actually contribute:
...
The refusal names the keys it blocks on and the linkage fields those keys cost, and points at the configuration the terms came from. Both surfaces read the same verdict, so what an operator meets here is the refusal `alcove exchange` would raise against the same CSV -- reached before the invitation is disclosed rather than after a partner has accepted it.

Whichever source supplies the terms supplies the identity in them. Inferring the terms from an `INPUT_FILE` requires `--identity`; taking them from a pre-existing configuration file mints the invitation under that file's `linkage_terms.identity`, and an `--identity` given there is reported as having no effect (edit `linkage_terms.identity` in the configuration to change it). Either way an invitation names its inviter: a configuration holding no identity is refused rather than minted under none, since `--identity` cannot stand in for a file that persists unchanged and supplies every later run.

When the configuration file's connection is `webrtc`, the invitation names that connection's coordination server and relay as its [connection endpoint](#invitation-strings), as an [online invitation over WebRTC](#inviting-over-webrtc) does: the relay is the `url` of each `turn` entry that sets no `username` or `credential`, and the `stun` list ([`invitation_relay`](EXCHANGE_REFERENCE.md#connectioninvitation_relay)). That connection block must be valid when the invitation is generated; any other channel's block may still be a placeholder. Because the invitation names a coordination server, a partner who accepts it with an `INPUT_FILE` runs the exchange at once ([Accepting and running a WebRTC exchange](#accepting-and-running-a-webrtc-exchange)): run `alcove exchange` at the same time, or have them accept without an input file and agree when both of you run it.
...
To withdraw a pending offline invitation before its nominal `expires`, delete the key file it wrote (`.alcove.key` at the default path, or the `--key-file` path). The offline key exchange completes only when the inviting party still holds the pending shared secret, so removing the inviter's copy invalidates the invitation: the secret held in the invitation string you forwarded can no longer authenticate a handshake against you, and the partner's copy is inert on its own without a live inviter to exchange with. Delete only the key file -- any configuration file (`alcove.yaml`) is left in place, so abandoning a pending invitation never disturbs a recurring exchange the same configuration still serves. The `invite` command prints this reminder, naming the key file, when it generates an offline invitation.

This is distinct from recovering a lost, reset, or compromised key (see [Recovery](#recovery)): it is the supported way to retract an invitation you have changed your mind about, not a response to exposure. Taking no action also closes the window -- the invitation lapses on its own at the `expires` shown when it was generated -- but deleting the key file closes it immediately rather than waiting out the lifetime.
...
`--identity` names this party in the terms the acceptance records. Without it, at a terminal, `accept` asks for the label once the invitation has decoded and before the terms are displayed -- the same interactive session that then shows them and takes the y/N -- and the answer is what the configuration it writes holds.

- **Where nothing can answer.** An acceptance with no label is refused (exit 64) rather than left waiting: with no terminal, and under [`--consent-to-terms`](#accepting-without-the-prompt), which declares the run unattended.
- **An existing configuration.** An acceptance that keeps an [existing configuration](#existing-files) asks nothing either, and needs nothing supplied: it proceeds under that file's own `linkage_terms.identity`, which is what the exchanges the file governs send.
- **A `--identity` given on such a run** is reported as having no effect, naming both labels and the field to edit. That notice travels with the terms display rather than as an ordinary diagnostic: it is reported before the terms, and where acceptance stops to ask, on the terminal it asks on whatever `--log-file` and `--log-level` are set to (see [Where the display is shown](#where-the-display-is-shown)) -- so on a run that stops to ask, the name your partner reads never diverges from the one the kept file holds without your seeing it first.
- **An unattended run** -- [`--consent-to-terms`](#accepting-without-the-prompt) at a `--log-level` above `warn` (`error` or `silent`) -- reports the notice nowhere; the run still proceeds under the kept file's label either way.
- **A kept configuration holding no label**, or still holding the placeholder, is refused (exit 64) rather than run unnamed.

`OUTPUT_FILE` is where the result goes, and belongs to an acceptance that runs its own exchange (see [Accepting and running a WebRTC exchange](#accepting-and-running-a-webrtc-exchange)); omit it and the result is written to standard output, exactly as `alcove exchange` writes it. An acceptance that writes only a configuration and key file has no result to send there and reports the argument unused rather than dropping it.
...
Every fact the display states includes the basis it rests on, so the two are never told apart by omission:

- `(enforced)` -- Alcove holds the fact itself. Either it is true of the run, or the exchange aborts rather than proceed without it.
- `(your partner's word)` -- the fact is what your partner declared. Alcove shows it faithfully but neither verifies nor enforces it, and a partner that does not honor it is not stopped by this tool.

A fact that needs more than the marker has its explanation on the line below it, in the same wording the web consent screen uses for the same fact.
...
An acceptance that runs the exchange itself (see [Accepting and running a WebRTC exchange](#accepting-and-running-a-webrtc-exchange)) says so above the outline, since on that path the confirmation is the last checkpoint before your data moves. Two lines state it: the coordination server the run resolves to dial, host and port, and what confirming does -- connect to that server immediately and run the exchange from your input file, transmitting your linkage data on the terms below it; declining writes nothing and connects to nothing. Under `--consent-to-terms` the same two lines state the run the recorded consent authorizes. An acceptance that writes a configuration and stops dials nothing and has neither line.

An acceptance that takes its exchange directories from the invitation's endpoint says so above the outline as well, because your partner chose those paths. It names the inbound directory, where you read your partner's files, and the outbound directory, where you write yours, and states what confirming does with them. On an [online acceptance](#online-acceptance) the run reads and writes the exchange files there and your configuration keeps them for later exchanges, and `--outbound-path` uses your own directories instead. On an offline acceptance the configuration it writes records them for `alcove exchange`, and editing that configuration's connection block before running it uses your own; an offline acceptance that keeps an existing configuration writes no connection and names none. Under `--consent-to-terms` the same lines state what the recorded consent authorizes, in the log at the default `info` level -- online, before the exchange opens either directory.

- **`columns you will send`** (`enforced`) -- the columns this party will send to your partner for matched records, one per line. It reads `(none)` in three cases where nothing leaves:

  - your file discloses no columns;
...
- **`the inviting party will receive the result`** (`enforced` on a `yes`, `your partner's word` on a `no`) -- the marker follows the answer, because the two are not alike. A `yes` is a disclosure the run itself makes, and what your partner does with the result once it holds it is governed by your agreement rather than by Alcove. A `no` is the withholding of a result from a partner, which rests on the agreed terms being honored, not on anything Alcove can impose.
- **`what your partner learns about its own records`** (`enforced`) -- shown when your partner does not receive the result of a `psi` exchange, in one of two readings that follow what the run does with your partner's half of the matched-pair table. Where the run returns that half, helping compute the match tells an honest partner which of its own records are in your data: inherent to a match that reveals identifiers, not a breach, and bounded -- never which of your records they met, nor anything about the rest of your set beyond its size. Where the exchange withholds it -- a `single-pass` run whose invitation declares no column for your partner to send, so it needs nothing back and the frame is suppressed -- your partner's process is never sent which of its own records are in your data, and that withholding is a limit of the exchange rather than a choice of the software your partner runs. A count-only (`psi-c`) invitation does not include the line at all: the party that receives no count computes none and is sent none, so it learns nothing about which of its records match, and what a count-only exchange does disclose is stated with the algorithm instead.
...
- **`allowed-character patterns`** (`your partner's word`) -- shown once when any field declares one, ahead of the fields, stating the caveat that covers every pattern listed below it: each is a partner-supplied regular expression Alcove has not verified. The patterns themselves are shown in full under the field each belongs to, so a reader who knows regular expressions can inspect what one actually admits.
- **`personal data used`** (`enforced`) -- the categories of personal data the keys are computed over. Under each, **`declared data standards`** (`your partner's word`) lists the standards your partner commits that category to, including any allowed-character pattern. Those are data expectations, not filters Alcove applies.
...
- **`columns you will receive`** (`enforced` where the invitation contains the column set, `your partner's word` where its terms only declare one, then how many columns the declaration holds) -- the columns the invitation says your partner will send for matched records, or `(none)` where it contains an empty set. Only a declared direction reaches this line: where the invitation declares nothing, the line is absent and what arrives is reconciled against your partner's own disclosure when the exchange runs. The line is absent too where `you will receive the result` is `no` -- a party sent no result is sent no columns with it, so nothing arrives for this line to count, whatever set the invitation contains. What your side locks in is the set the invitation *contains* -- the exact columns your partner's own disclosure rule produced -- and your own side is what enforces it: anything else your partner sends is received and then rejected, aborting the exchange. A contained empty set is that same commitment with no column in it.

  Where the invitation contains no such set, and the columns shown come from the terms your partner authored, the marker says so: there is no recorded set to hold them to, so an online acceptance reconciles what arrives against your partner's own disclosure instead. The abort stops the exchange from completing, not the columns from crossing: each side reconciles what it received only after the payloads have been exchanged, so a violated commitment is caught on the far side of the wire.
...
- **`legal agreement`** (`your partner's word`) -- the reference, the stated purpose, and the date the agreement is valid through, when the invitation attaches one. The reference and the date are byte-compared against your own copy before data moves; the text itself is your partner's, never vetted by Alcove.
- **`expires`** (`enforced`) -- the instant past which the invitation is refused, when the token holds one.
...
- **Declared, and refused.** A count-only exchange counts matched values where the matching pairs each record at most once, so it refuses a term that expands one value into several match candidates -- an approximate-match setting, or a swap matching two elements in either order -- rather than running the terms with a narrower match. Each such term is shown with a note stating that refusal and what to ask your partner for, so the display never states behavior the run does not perform.
- **Applied, and it discloses more.** Duplicate matching is a term the run does honor, and it discloses more than a one-to-one match does, so the `duplicate matches` line includes a sentence stating what -- and which sentence follows the invitation's output direction, since that decides who reads the grouping. Where the invitation shares the result with you: your side learns, for each of your own matched records, how many of your partner's records group onto it and which rows they are. Where your partner is the sole receiver: it reads that grouping about its own records, you are sent no result, and Alcove shows you no group sizes and no row positions. The limit is stated with it: the matching rounds do send that grouping to your machine, so what you are shown is Alcove's own doing rather than something the wire holds back.
...
- **Not recognized.** A value transform this version does not recognize is marked as unrecognized rather than printed in the same shape as a rule Alcove can explain.

The configuration route the duplicate-matching note names is `alcove exchange` on both sides, each party's `deduplicate` coming from its own configuration file: your `true` against your partner's `false` groups yours alone, and both parties declaring `true` groups both -- a many-to-many match, which runs under `linkage_strategy: cascade`. Accepting an invitation on the command line reaches neither: `alcove accept` offers no control for your own side, so it stays `false` whatever the invitation declares. The web app's accept screen does offer it, and sets it on the terms that party presents. See [`linkage_terms.deduplicate`](EXCHANGE_REFERENCE.md#linkage_termsdeduplicate) for what each pair matches and what a both-sided match hands you.
...
- **Default behavior and logging.** It is off by default: without it, `accept` displays the terms and stops to ask you to confirm them. Since nothing asks, there is nothing to show alongside a question: the terms stay diagnostic output on the routing you chose, so `--log-file` captures them for the unattended run's record and `--log-level silent` drops them.
- **Standard input.** It also frees standard input for a `-` `INPUT_FILE` (see the `-` standard-input note under [Configuration](#configuration)).
...
- **The input cannot produce a key's fields** -- the columns the key needs are absent, or have a type the terms' cleaning cannot bind. The error names the unsatisfied fields and the keys they cost. The remedy is a CSV covering those field types, or new terms.
...
- **The input discloses columns the invitation will not accept** -- the invitation declares the inviting party accepts no payload column *and* is entitled to the matched result, while the input discloses some (the same set the display's `columns you will send` line lists, on an acceptance that reaches that display). A warning names them, one per line, because that disagreement is one the exchange refuses to run on. The two remedies are to set those columns not to transmit in the written configuration (`is_payload: false`, or the `ignored` role), or to ask the partner for an invitation that accepts them. What follows the warning differs by path, and the warning says which:
  ...
  - An acceptance that only writes a configuration prepares nothing, so it is not stopped: confirming writes the configuration and key file, and the refusal arrives when you run `alcove exchange`.
...
An invitation whose [connection endpoint](#invitation-strings) names a WebRTC coordination server holds everything an exchange needs -- the coordination server, the shared secret, and the agreed terms -- and the accepting side takes the `acceptor` end of the rendezvous from the command it runs, so nothing is left for you to fill in. Given an `INPUT_FILE`, acceptance therefore accepts *and* runs the exchange in one invocation, writing the same configuration, key file, audit record, and result that an `alcove accept` followed by an `alcove exchange` writes. That is what lets the partner of an [online WebRTC invitation](#inviting-over-webrtc) meet the inviter inside its accept-timeout without a second command.
...
`--peer-timeout` bounds this leg, and is the only setting that does. One value replaces all three of the transport's waits -- meeting the partner at the rendezvous, the data channel opening once both sides are present, and peer silence on the open channel after -- so size it for the whole run rather than for the arrival alone. A partner who never arrives ends the command at that bound with exit 69 (`EX_UNAVAILABLE`, see [Exit codes](#exit-codes)), which is what lets an unattended acceptance fail fast instead of waiting out the defaults: ten minutes to meet the partner, then an hour of peer silence, when the flag is absent. One thing outlasts every budget -- a run that allocated against a [relay](#turn) leaves an allocation-refresh timer armed that its teardown cannot cancel -- but the command still returns once the run's own work is done, naming what was left open as it goes.

The value is also recorded as `connection.options.peer_timeout_ms` in the configuration this acceptance writes, so every later `alcove exchange` from that file waits the same. A short window chosen for one interactive setup is a short window for each recurring run until the field is edited.
...
- **No `INPUT_FILE`.** There is no dataset to exchange, so acceptance writes the configuration and key file; run `alcove exchange` with your input file afterward.
- **An invitation holding no WebRTC endpoint.** An `sftp` or `filedrop` endpoint, or none at all, leaves a connection block whose credentials you still supply, so acceptance writes it for you to complete.
- **A reused configuration.** A configuration already at the configuration path governs its own exchange -- `alcove exchange` loads it and dials what it says, including its `@path` references, `server.key`, and `secure` -- so acceptance writes the key file and stops.

In all three, `--peer-timeout` is reported as having no effect, along with the other connection-options overrides: the connection block those acceptances write is one you complete by hand, so the budget goes under `connection.options` in that file.

After an acceptance that writes only a configuration, both parties run `alcove exchange` at their convenience.

## Online invitation

```sh
alcove invite --identity IDENTITY [--accept-timeout=DURATION] [--expires-in DURATION] [--linkage-strategy STRATEGY] [--turn URL]... [--stun URL]... URL INPUT_FILE [OUTPUT_FILE]
```

Similar to [offline invitation](#offline-invitation), this generates a shareable invitation string (see [Invitation strings](#invitation-strings)) then prints it and instructions for the user to forward to their partner by a secure, out-of-band channel. Those instructions include a template for the invocation of `alcove accept` that references the shared server (over WebRTC there is none for the partner to type -- see [Inviting over WebRTC](#inviting-over-webrtc)). The template names the invitation as `<INVITATION>` beside its `<INPUT_FILE>` placeholder rather than including it: the invitation encodes the setup shared secret, so its delivery is the stdout line above -- which the operator directs -- while the template is a diagnostic like every other line, routed to stderr or to a `--log-file`. The invitation it prints also embeds a [credential-free connection endpoint](#invitation-strings) derived from the connection this invite is using -- the public locator only (host/port/path, or the split `inbound_path`/`outbound_path` pair), never credentials -- so an acceptor seeds its `connection` block from it and need only supply its own credentials. After printing the invitation information, the program connects to the server and waits for the partner to respond.

### Inviting over WebRTC

An invitation over the [`webrtc` channel](#webrtc-exchanges) names the PeerJS peer-coordination server the two parties meet through. The command takes it in one of two forms:

- **The web app's address**, the one you send a partner who uses the browser:

  ```sh
  alcove invite --identity "Agency A" https://app.example.org/ input.csv
  ```
...
- **The partner needs no URL, and no second command.** The printed template is `alcove accept --identity <YOUR NAME, YOUR ORGANIZATION> <INVITATION> <INPUT_FILE>`, run while this command is still waiting: accepting writes the connection block (coordination server and `role` both) from the endpoint and dials it in the same invocation (see [Accepting and running a WebRTC exchange](#accepting-and-running-a-webrtc-exchange)). The `role` is not included in the invitation -- each command stamps its own end, `inviter` here and `acceptor` there. The partner replaces the placeholder with their own name and quotes it themselves if it contains spaces or commas.
- **The URL names the location and nothing else.** A `ws://`/`wss://` URL holding a non-empty user, password, query, or fragment is a usage error (exit 64) rather than a silently dropped component, as is one whose host or path could move the signaling socket elsewhere (an `@`, `?`, `#`, `\`, or space, however it was written) or whose port is not dialable. Every such refusal lands before the invitation is minted, so an unusable URL never costs a printed token. For a coordination server that needs an API key, author `channel: webrtc` with `server.key` in `alcove.yaml` and use [`alcove exchange`](#webrtc-exchanges) instead.
...
- **`--turn URL` and `--stun URL` name this party's relay.** Each takes one url in the grammar of [`connection.turn`](EXCHANGE_REFERENCE.md#connectionturn)'s `url` and [`connection.stun`](EXCHANGE_REFERENCE.md#connectionstun); repeat a flag to name several. The invitation names them for a partner with no relay of its own ([`invitation_relay`](EXCHANGE_REFERENCE.md#connectioninvitation_relay)), and the configuration saved at acceptance holds them as url-only `turn` entries and a `stun` list, so every run of this party mints the TURN credential from the shared secret (see [TURN](#turn)). A `--turn` relay must accept those credentials; a url holding a user, or a query parameter other than `transport`, is a usage error (exit 64) before the invitation is minted. A `--stun` list replaces the built-in default STUN server. On an `sftp://`, `ssh://`, or `file://` URL, and on an offline invitation, both flags are reported ignored.
...
- **No `--server-*` flag applies.** Every one of them is reported ignored, credentials included: the coordination server's port is part of the URL, and a webrtc invitation reaches it by location and its API key alone, so nothing typed at `--server-password`, `--server-private-key`, `--server-private-key-passphrase`, `--server-keyboard-interactive`, or `--server-host-key-fingerprint` is sent, saved, or included in the invitation. A coordination server that needs a key takes it from `server.key` in `alcove.yaml`; `server.username` has no consumer on this channel -- the schema accepts it, but only `server.key` is honoured.
...
`--accept-timeout` is this run's peer budget, bounding both the wait for the partner to accept and the peer waits of the exchange that follows. It bounds that run only: it is not written to the saved configuration, so a later `alcove exchange` from that configuration runs on its channel's own defaults rather than a window sized for one operator waiting at a terminal. Which defaults those are depends on the channel:
...
`--peer-timeout` is how those later runs get a budget other than their channel's defaults. It does not bound this invitation's wait -- the command reports that when the flag is set, naming the timeout that governs the phase instead -- and is recorded as `connection.options.peer_timeout_ms` when the configuration is saved. Editing that field in the file before a later `alcove exchange` does the same thing by hand.

Once the configuration is written the command reports which of the two it holds: the `--peer-timeout` value, or no field at all, in which case it names the channel defaults that leaves later runs on.
...
```sh
alcove update [--config-file FILE] [--key-file FILE]
alcove apply [--config-file FILE] [--key-file FILE] UPDATE
```

Once a partnership is set up with `alcove invite` and `alcove accept`, either party can change its linkage terms or the columns it discloses without re-inviting: no new shared secret, and no change to either party's connection block. One party edits its own configuration and runs `alcove update`; the other runs `alcove apply` on what it prints.

### Making an update

1. **Edit your configuration.** Change `linkage_terms`, or the `metadata` that decides which of your columns are disclosed, in the `alcove.yaml` you exchange with this partner.
2. **Run `alcove update`.** It reads that configuration and the key file beside it, and prints a terms update to stdout. The update holds your linkage terms and the names of the columns you disclose, authenticated under the shared secret in your key file. It holds no secret, no credential, and no connection details.
...
```sh
alcove exchange [--invitation CODE] [--sweep-exchange-files [--force-retain-sweep]] INPUT_FILE [OUTPUT_FILE]
```
...
- the rule set your terms cite, where Alcove ships that set and the keys or fields declared for it are not drawn from it. The two cited halves are named under what this build found for each, beneath the keys the finding is about, and the remedy is the one you can act on: restore the rules the cited set declares, or drop the citation. A citation the declared rules do support, and one naming a set Alcove does not ship, add no line -- neither leaves you anything to correct. The same divergence is reported once more when the configuration loads, naming the file and the field ([A rule-set citation that no longer fits](#a-rule-set-citation-that-no-longer-fits)); the display states it whatever your `--log-level`.
...
All three channels -- `sftp`, `filedrop`, and `webrtc` -- run here. For file-drop exchanges, the `alcove.yaml` configuration uses `channel: filedrop` and `path` in place of `channel: sftp` and `server`:
...
- **`role` is required** and the two parties must differ: one `inviter`, one `acceptor`. Each registers with the coordination server under the id its role derives from the shared secret and dials the id the other's derives, so neither needs the other's address. `alcove invite` and `alcove accept` stamp it; a configuration missing it is a usage error (exit 64) before anything is dialed, and one where both parties set the same value fails at the coordination server with an error naming that as the likely cause.
...
- **Where the connection comes from.** Accepting an invitation seeds the whole block -- coordination server and `role` both -- from the invitation itself, so `alcove accept` writes a ready configuration and, given an input file, [runs the exchange on it in the same invocation](#accepting-and-running-a-webrtc-exchange). That holds for an invitation minted by the web application and for one minted by [`alcove invite` over a `ws://`/`wss://` URL](#inviting-over-webrtc). Otherwise author it by hand: a `ws://` or `wss://` URL is refused (exit 64) on `alcove accept` and on the [zero-setup exchange](#zero-setup-exchange), which take their end of the rendezvous from an invitation they were given, not from a URL.
...
Two things follow from it. A run so configured has no direct path to fall back to, so a relay this host cannot reach ends the run rather than degrading to a direct connection -- reported as [no relay candidate gathered](#when-a-webrtc-exchange-does-not-connect), which on this setting is the whole story. And setting it with no `turn` entry -- or with one whose `url` names no host, which is no relay either -- is a usage error (exit 64) before anything is dialed, since a relay-only run with no relay could gather nothing to pair. A `url`'s `transport` parameter is held to the same standard, because the ICE layer drops an entry whose transport it cannot use and continues without it: leave `transport` unset or set it to lowercase `tcp`, or to `udp` on a `turn:` url, and any other value -- `TCP`, `quic`, an empty one, or `udp` on a `turns:` url -- is refused at parse rather than leaving the run to gather host addresses. It is this party's own setting: it constrains only what this host gathers, and the partner is free to offer whatever its own network allows. It lives in the connection block an `alcove exchange` run reads: an acceptance that runs the exchange from the partner's invitation with no kept configuration composes its connection from the locator, which never holds this setting, so that run gathers host candidates like any other -- relaying through the partner's relay where the invitation names one ([`invitation_relay`](EXCHANGE_REFERENCE.md#connectioninvitation_relay)). To keep an accepted exchange off a direct path, add the setting, and a `turn` entry where the invitation names no relay, to the configuration `alcove accept` writes and run it with `alcove exchange`.

**The run says which policy it applied.** Every WebRTC run states it as it opens the rendezvous -- `ice_transport_policy: relay (relay candidates only)`, or `all by default` where the connection sets none -- so the run's own log -- a `--log-file` included -- records which candidate types the exchange gathered. A relay-only run that gathers no relay candidate names the policy in the failure as well.

**A key the connection block does not define is refused.** `ice_transport_polcy` for `ice_transport_policy`, or a `server` key written beside it rather than under it, is a usage error (exit 64) naming the key, before anything is dialed. A setting that decides which addresses this party offers cannot be dropped in silence and leave the run on a policy you did not choose.

**A run that allocates against a relay leaves a timer armed behind it.** The allocation a relay hands out is held open by a refresh timer the exchange's teardown cannot cancel, and it stays armed for five sixths of the lifetime the relay granted -- roughly eight minutes where a relay grants the usual ten. What it holds is the event loop rather than the command: the result file, the exchange record and the receipt are all written before it, nothing is transferred during it, and the process [returns once that work is done](#logs-and-what-the-exit-code-tells-the-scheduler), naming the resource kinds still open on stderr. The mechanism and its measurement are in [WEBRTC_TRANSPORT.md](spec/WEBRTC_TRANSPORT.md#budgets).
...
The failure states whether a relay candidate was gathered, how many remote candidates the partner sent, and how many pairs were tried. A report of no relay candidate is the first case or the second according to your own `turn` configuration, which the failure does not read. It does read [`ice_transport_policy`](#turn) and names it where a relay-only run gathered none, the case with no direct path left to fall back on. A completed exchange reports the candidate pair it settled on at `--log-level debug`, which is what tells you whether a run that worked went direct or through the relay.
...
**A run configured for an environment proxy is told no certificate was checked.** The check dials the signaling server itself and never through a proxy, so on a `wss://` run that turns Node's environment proxying on -- `NODE_USE_ENV_PROXY` set to `1`, or `--use-env-proxy` on the command line or in `NODE_OPTIONS` -- with a proxy address in `HTTPS_PROXY`, `https_proxy`, `HTTP_PROXY`, or `http_proxy`, it cannot tell whether the failed connection went through the proxy, and no certificate verdict is reported at all: the failure states that, and names the same remedy, since an intercepting proxy is the likely cause there too. A run whose `NO_PROXY` or `no_proxy` excludes the signaling host is answered the same way, since that exclusion applies per dial and the check does not read it.

### Sweeping a stale exchange directory

A crashed or mismatched prior run can leave protocol files in an `sftp`/`filedrop` directory that stall the next rendezvous. `--sweep-exchange-files` deletes every protocol file in the directory before the rendezvous -- this party's and the peer's hellos, acks, locks, joining sentinels, and messages -- and starts a fresh exchange. Foreign (non-protocol) files are never touched. It is accepted by `alcove exchange` and the zero-setup form, is CLI-only, and is never persisted to `alcove.yaml`. Confirm that no other session is using the directory before passing it: a sweep during a live exchange destroys that exchange's state.
...
An exchange writes its matched records to the `OUTPUT_FILE` path it was given, and to `stdout` when it was given none -- on `alcove exchange`, the zero-setup form, an online `alcove invite`, and an [acceptance that runs its own exchange](#accepting-and-running-a-webrtc-exchange) alike. What that file holds, left to right:
...
Under `signing.mode: certificate` the run loads this party's signing identity from the path `signing.identity_file` names, before any credential, terms, or data are sent. A configuration that names none is refused earlier still, from the configuration alone -- before the run prepares your input or reaches the server -- and exits 64: Alcove chooses no location for a signing identity, since it is a long-lived credential whose custody is yours (see [Where the signing identity lives](#where-the-signing-identity-lives)). Set `signing.identity_file` -- `/run/signing/alcove-signing-identity.json` is the usual shape, created there by `alcove fingerprint --identity-file` -- or run `signing.mode: none` unsigned. A path that is set but holds no identity file exits 64 too, naming that path.

A partner verifies a receipt against the identity in the agreed terms rather than the one the certificate holds, so an identity bound to anything other than the run's `linkage_terms.identity` -- or the `--identity` that replaces it for that run -- signs receipts the partner rejects. That configuration is refused where the identity is loaded: the run exits 64, naming both values and the two ways to reconcile them, before anything is sent. Reconcile them with [`alcove fingerprint --force --identity`](#signing-identity-fingerprint) naming the terms identity -- which changes the fingerprint your partner has pinned, so it needs a coordinated re-pin -- or by editing `linkage_terms.identity` to the bound value. A bound label longer than the terms' length bound is not covered by that guidance and keeps the older remedy; no current Alcove build can bind such a label, so it is a stated limit rather than a path.

Such a run has no way to finish, and refusing it up front is what spares you the connection, the credentials it presents, and the terms it puts on the wire. The exchange holds the same binding itself, as a safety check: a diverging run that gets as far as agreeing terms with your partner ends right there -- before any linkage key or data crosses, with no result and no receipt -- and ends the partner's run promptly too, rather than leaving it to time out.

### Pinning the partner's certificate

Both parties present their signing certificate to each other during the authenticated setup step that opens the exchange, before the first linkage key or payload row crosses. What your side does with the one it receives depends on whether `signing.partner_fingerprint` is already set.

- **It is set.** The presented certificate must match it. A partner presenting a different certificate ends the run right there, with no result, no receipt, and nothing disclosed -- and your configuration file is left exactly as it stands. The match reads the certificate's body alone: one that matches the pin but fails its own signature, or names a party other than the one your partner agreed terms under, is refused at the receipt swap, after the linkage keys and payload have crossed. Re-pinning is an edit you make, after confirming the new value with the partner out of band.
- **It is not set.** This is your first exchange with this partner, so the run pins the fingerprint it was presented, records it in the configuration file you passed to `alcove exchange`, and prints it. It pins only a certificate that verifies under its own key and names the party your partner agreed terms under; one that fails either ends the run right there, with nothing pinned and nothing disclosed. The line names the value and the file it went into, and it also rides the [machine-readable event stream](#machine-readable-event-stream) under `partnerCertificatePinned`, so a scheduled run that discards stderr still reports it. That write replaces the file by renaming a new one over it, so it needs the directory holding the configuration writable, which a read-only configuration mount does not give: such a run exits 64 before it connects, naming the file and telling you to set `signing.partner_fingerprint` yourself or mount the configuration writable for the first run (see [DEPLOYMENT.md](DEPLOYMENT.md#key-file-permissions-in-containers)).

A first-contact pin is only as trustworthy as the channel your invitation and shared secret travelled: whoever could have substituted those could have substituted the certificate pinned here. **Compare the printed fingerprint with the one your partner's [`alcove fingerprint`](#signing-identity-fingerprint) prints, over a channel you trust, before you rely on the receipts.** Setting `signing.partner_fingerprint` yourself before the first run avoids the question entirely: the run then holds the partner to the value you obtained out of band. Nothing about this is asked interactively -- a scheduled exchange pins and reports without a terminal attached.

Two other partner-certificate outcomes end the run at the same point, with nothing disclosed and the exit code of a failed exchange (69): a partner that presents no certificate, which means it is not signing receipts and no receipt can be produced, and a certificate this build cannot read. Both name what to ask the partner for.
...
Every command that opens an SFTP connection -- `alcove exchange`, an online `alcove invite`/`accept`, and a zero-setup exchange -- verifies the server's SSH host key before sending any credential, so a man-in-the-middle or a substituted server is detected rather than trusted. You can pin the key out-of-band by setting `connection.server.host_key_fingerprint` to the server's OpenSSH SHA256 fingerprint (the value `ssh-keygen -lf` prints; `@path` is supported). If you cannot obtain it out-of-band, the first interactive run establishes it on first use, the way `ssh` does:

- The first time you connect to an unpinned SFTP server from an interactive terminal, the command shows the presented host key's fingerprint and asks you to confirm. On confirmation it records the fingerprint and connects; every later run then verifies it silently. Verify the fingerprint against the server's published value if you can before confirming. Reading the key takes a single connection attempt, bounded by `--connection-timeout` (`connection.options.server_connect_timeout_ms`), and is not re-dialed: if it fails, the command stops before prompting and sends no credential, so run it again, raising the timeout if the server is slow to answer. Where the command writes a configuration -- `exchange` (into the existing `alcove.yaml`, preserving your comments), the online `invite`/`accept`, and a zero-setup run with `--save` -- the pin is saved there; a zero-setup run without `--save` trusts the key for that one exchange only (like `ssh` to a host you do not add to `known_hosts`).
...
- If the server legitimately rotates its host key, a later run fails with a mismatch error rather than silently trusting the new key. Verify the new fingerprint out-of-band, then re-pin: set `host_key_fingerprint` to the new value, or remove it from `alcove.yaml` and run once interactively to confirm and re-pin (the same as removing a changed host from `~/.ssh/known_hosts`).
...
```sh
alcove probe-host-key SFTP_URL [--json] [--connect-timeout <duration>]
```
...
Both lines name the program by absolute path. A scheduled job runs under the scheduler's environment rather than your shell's, so a bare `alcove` or `docker` may not resolve there -- under cron's minimal `PATH`, or a Task Scheduler service account's -- and the job then fails without ever reaching Alcove. Give the full path, or put the program on the scheduling account's `PATH`.
...
Diagnostics go to `stderr` and the result to `stdout`, and a scheduler disposes of both in its own way, so capture what you need. `--log-file PATH` appends timestamped lines to a file Alcove creates owner-only, and its parent directory must already exist (see [Logging](#logging)). `--log-level` still governs that file: `error` or `silent` discards precisely the notices a schedule depends on, including the over-permissive key-file warning and the expiring-soon advisory. `silent` also discards the notice of a transport that did not finish closing, which reaches no other channel. A supervising process that would rather not parse log lines can read structured progress and outcome events on file descriptor 3 with [`--event-stream`](#machine-readable-event-stream).

The command returns promptly once its work is done. Every file it writes to a path -- the result, the exchange record, the receipt -- is written and waited for with no time limit. Two waits are bounded instead. The transport closing is one, at a ceiling set per channel and well above that channel's own teardown budgets. It runs last, after the run has reported its outcome, so a close that reaches that ceiling costs the wait and nothing else: Alcove states on stderr how long it waited and what was still open, at error level so a run set to `--log-level error` still shows it, and leaves the exit code the exchange's own. On a file-drop or SFTP exchange that deletes its protocol files it also tells you to check the exchange directory for any the abandoned close left there. It tells you the run's own files are on disk only where they are, and what it accounts for is the result, the exchange record, the receipt, and the configuration a setup run writes after the exchange: a run that failed before its output stage or inside it, and one that completed but lost one of those, are told nothing about them, so the notice never sends you looking for a file no run produced. A configuration write lost earlier -- the one an [online invitation](#online-invitation) or [acceptance](#online-acceptance) makes when the partner accepts -- is outside that account and is reported where it happens, by its own warning and exit 73, so a run can report that loss and still be told its files are on disk. The other bounded wait is a result sent to `stdout` rather than to an `OUTPUT_FILE` path: Alcove waits for whatever is reading the pipe to take the last line, and gives up only once 60 seconds pass with none of the result leaving the process. A command reading a large result slowly is on no clock at all -- each part of it that leaves starts that minute again -- so what the bound catches is a reader that stopped. That reader, and one that closes the pipe before the last line (`| head -1` on a result past the pipe's buffer), both end the run the same way: exit 73, a result only partly delivered, the exchange record and receipt still written, and whatever the run would have saved after the result -- a `--save` configuration and key file among it -- left unwritten and named as skipped. That is the code a result file that could not be written gets, and the same instruction with it, since the exchange already happened and re-running conducts a second one. After that the process returns within three seconds, so a scheduled run does not hold its slot, or its container, past the exchange. If something inside it is still holding the process at that point, Alcove names the kinds of resource still open on stderr and exits with the status the run already reached -- the line reports a fault to pass on, and never changes the exit code a supervisor acts on.
...
```sh
alcove fingerprint [--identity-file PATH] [--identity STRING] [--config-file PATH] [--force] [--export-certificate PATH]
```
...
Creation is announced rather than silent. The identity string bound into the certificate -- the party's name, organization, and contact -- comes from `--identity`, or from `linkage_terms.identity` in the configuration when the flag is absent; once an identity exists, `--identity` is ignored unless `--force` is also given. A label containing a control character, one of the nine Unicode text-direction formatting characters, or a PEM or OpenSSH private-key block, or one past the length the linkage terms hold the same value to, is refused rather than bound: the value goes into a certificate a partner pins and reads back long after the run, a text-direction character in it reorders the copy it is displayed beside, and the linkage terms refuse a key-bearing identity outright, so a certificate bound to one could never be named by the terms a run agrees under. The implicit direction marks `U+200E`, `U+200F`, and `U+061C` are admitted, as they are in the linkage terms.

### Where the signing identity lives

You choose, and Alcove never does. The path comes from `--identity-file`, or from `signing.identity_file` in the configuration named by `--config-file`; with neither, the command refuses and prints nothing on stdout, so a captured `FP=$(alcove fingerprint)` is empty and the status nonzero rather than a fingerprint minted somewhere you did not pick.
...
```sh
alcove fingerprint --identity-file /run/signing/alcove-signing-identity.json --identity "Agency A, a@agency-a.gov"
```
...
- **Writable for the run that creates the file, read-only after.** Only `alcove fingerprint` writes the identity; an exchange and an `alcove verify-receipt` read it and write nothing beside it, so a read-only mount serves every run after the creating one.
...
A `~`-relative path is expanded, so naming one under your home directory works verbatim -- what Alcove does not do is choose the home directory (or any other location) for you. In a container that home is typically ephemeral, and a key minted there is a fresh key with a fresh fingerprint on every run.
...
Bind the identity that matches `linkage_terms.identity` in your configuration. A partner verifies a receipt against the identity in the agreed terms rather than the one the presented certificate holds, so a certificate bound to any other string signs receipts the partner rejects. Binding one that diverges from the configured value warns and proceeds -- this command sends nothing, and editing `linkage_terms.identity` to match is the other way to reconcile them -- but bring the two into agreement before you share the fingerprint. An exchange under a divergent pair does not run at all: [`alcove exchange`](#signing-identity-and-the-agreed-terms) refuses it.
...
```sh
alcove verify-receipt RECORD [INPUT_FILE] [RESULT_FILE]
```
...
- The **exchange record** (`alcove-record-<stamp>.json`), self-attested and unsigned. Verifying it is an internal-consistency check: it says nothing about the partner.
- The **dual-signed record** (`alcove-receipt-<stamp>.json`), written when the exchange ran with `signing.mode: certificate`. This is the evidence against the partner: both parties' signatures and certificates over the terms and the data that flowed, beside a copy of the partner's linkage terms that no signature covers and verification re-derives the agreed-terms hash from.
...
The record holds no matched data -- only salted commitments to it -- so verification **re-supplies** the committed data from your own retained files: pass the `INPUT_FILE` you contributed and the `RESULT_FILE` you kept, and the command reconstructs the committed data and opens every commitment (the sent payload, the received payload, and the record's pairing). Reproduction is byte-exact only from **unmodified** retained files -- a results file re-sorted or re-exported in a spreadsheet will not reproduce -- and a duplicate value in an identifier column is reported as a note. A received cell the partner committed as null is a known limitation: the result writes a null and an empty string the same way, so the cell reproduces as an empty string and the received-payload commitment reports a mismatch (never silently mis-opened). Whenever that commitment mismatches and the re-supplied received payload has an empty cell, a note names the empty-versus-null difference as a possible cause -- a cause to check, not an exoneration: the verdict stays a mismatch, since the record cannot say which of the two was committed.

The **result size** the record states is checked too, by recount rather than by opening: it is the number of matched pairs, so when the record's pairing opens against your re-supplied result the command recounts that pairing and compares the two. A recorded size the opened pairing does not hold is a definite failure on its own line, distinct from a commitment mismatch and not ambiguous the way one is -- a result that did not belong to this exchange fails the pairing's own line first, so it never reaches this one. Where no pairing stands behind the figure -- you supplied no result, the keys hold no salt for it, it did not open, it opened but is not shaped as a pairing and so holds no count to recount, or the record holds none at all (a count-only exchange records no pairing) -- the size is reported as not checked. A record that states no size at all, which is every exchange where only one party received output, gets no line and is not at fault for it.

With both parties' linkage terms in hand it also re-derives the agreed-terms hash. `--config-file` (your exchange config) supplies yours; your partner's are the copy the dual-signed record holds, so naming that receipt supplies them. `--partner-terms` supplies a document in that copy's place -- for a receipt whose copy was stripped, or a run verifying an exchange record with no receipt beside it. Without both halves, the terms hash is reported as not checked. `--config-file` is never auto-loaded -- name it explicitly, since a stray config in the working directory may belong to a different exchange.

Either path is one you named, so a file that is not there is a usage error rather than a run with the terms hash quietly left unchecked. A `--partner-terms` file that defines no `linkage_terms` is refused for the same reason -- supplying the partner's terms is all that flag does -- and a file you do name is what the check runs against, in place of the copy the receipt holds. A `--config-file` that defines none is not: the same config is where `signing.partner_fingerprint` and `signing.identity_file` are read from, so one holding only those is accepted for them, and a note beside the agreed-terms line records that the config supplied no terms. A "not checked" line names what is still missing, so a config you already passed is never the remediation it points you back at.
...
- **not checked** -- no exchange record was named. Nothing is contradicted, so the run is not a failure, but the verdict stays `INCOMPLETE` (exit 66): which run the receipt attests is open. A dual-signed record verified on its own therefore reports `INCOMPLETE` even with both certificates anchored and the terms hash checked. That is the accurate reading rather than a gap -- name the run's exchange record as `RECORD` and pass the receipt to `--signed-record` to pair them.
...
- **`alcove exchange` is what signs.** The receipt is bound to the session key the authenticated key exchange produces, and the zero-setup form has neither that nor a configuration it reads, so signing starts at the recurring command the save set you up for.
...
The identity is a long-lived P-256 private key and its self-signed certificate, reused with every partner, so it belongs where your other credentials live rather than in the exchange's directory (see [Where the signing identity lives](#where-the-signing-identity-lives)). [`alcove fingerprint`](#signing-identity-fingerprint) creates it, at the path you name:
...
- **The command announces what it did**, reporting the action, the path, and the bound identity on stderr. The 43-character fingerprint goes to stdout alone, so `FP=$(alcove fingerprint --identity-file /run/signing/alcove-signing-identity.json)` captures the value and nothing else.
...
Add the block to the `alcove.yaml` the save wrote:
...
On a partnership bootstrapped with `--save` that channel is the zero-setup exchange itself: the secret crossed under the transport's protection alone -- the SFTP server, or the access controls on the shared directory -- so the pin rests on the same trust in whoever administers that server or share (see [Bootstrapping a shared secret](SECURITY_DESIGN.md#bootstrapping-a-shared-secret)). On a partnership established by [`alcove invite`](#offline-invitation) and [`alcove accept`](#offline-acceptance) it is the out-of-band channel the invitation was forwarded over.

Either of two actions anchors the pin outside that channel, and one is enough:

- **Compare the printed fingerprint with your partner**, over a channel you trust, before you rely on the receipts. Their value is what `alcove fingerprint` prints on their side.
...
- **The agreed-terms hash is re-derived from both parties' terms**: `--config-file` supplies yours, and the receipt holds your partner's, so the command above checks it from the files this exchange already wrote, with no terms document to keep beside them. `--partner-terms` supplies that half for a receipt that holds none; without it such a run reports the terms hash as not checked, and the verdict stays `INCOMPLETE` (exit 66).
...
When [`authentication.token_max_age_days`](EXCHANGE_REFERENCE.md#authenticationtoken_max_age_days) is set, each rotated token is stamped with an expiry that many days out, so a token cannot outlive the configured maximum age between exchanges; without it, rotated tokens do not expire. `alcove exchange` checks the token's age at load time, before opening any connection. An already-expired token aborts the run with an error naming the expired time and directing both parties to re-invite (no key exchange is attempted); this applies to any token holding an `expires` -- including an invitation token's bounded lifetime -- independently of `token_max_age_days`. When `token_max_age_days` is set, a token within `token_max_age_days / 3` days of expiry additionally prints a warning before the exchange. The warning is suppressed when that exchange succeeds, because rotation refreshes the expiry; it appears only when the token was not refreshed, as a prompt to re-invite before it lapses. A stamped `expires` is honored regardless of whether `token_max_age_days` is still set, so removing the field does not revive a token that has already passed its expiry.

### Out-of-sync tokens

If one party fails to write the rotated token to its key file - whether due to a crash, power loss, or a disk error - the two sides will hold different tokens and the next key exchange will fail. Both the run whose save failed and every later run that fails authentication exit 77, which a scheduled run's supervisor can act on without retrying (see [Exit codes](#exit-codes)). Clock skew can produce the same result: if one party's clock lags and a token expires between the key-exchange round-trip messages, that party fails the post-handshake expiry check and discards the new token while the other party saves it successfully. Because there is no way to determine which party holds the newer token, both must reset regardless of which side failed; reusing an older token may also violate key-rotation policies.
...
1. **Both parties remove their key file** (`.alcove.key`, or the `--key-file` path). The offline `invite` and `accept` used below each refuse to overwrite an existing one with a usage error (exit 64), so the out-of-sync token has to go before either command will run.
2. **The inviting party runs `alcove invite`** -- the offline form, with no URL. Its pre-existing `alcove.yaml` supplies the linkage terms; the command writes a fresh key file and prints a new invitation to forward out-of-band.
3. **The partner runs `alcove accept INVITATION [INPUT_FILE]`**, which reuses their own `alcove.yaml` the same way and writes their key file.
4. **Both parties run `alcove exchange`** at their convenience, as for any [recurring exchange](#recurring-exchange).

Each side's configuration is reused throughout; only the key file is recreated. Neither command takes `--identity`: each runs under the label its kept configuration already holds -- the one the partnership has been exchanging under all along -- and a flag typed at either is reported as having no effect rather than renaming the party for the recovery run (see [Configuration](#configuration)).
...
Before its key exchange starts, `alcove exchange` records in the key file that a rotation is in flight, and the write that saves the rotated token removes that record again. So does a key exchange that fails authentication: this side did not rotate, and that failure is what you act on, so confirm with your partner first as the section above says (your partner may have saved a rotated token before the failure). A run stopped between the two -- killed, out of power, or losing its connection partway through the key exchange -- leaves it in place, so the next run starts with a warning: an earlier key exchange began at the time it names and did not save its rotated token, and your partner may hold a token this key file does not.
...
Alcove follows the standard stream convention: a command's result data goes to `stdout`, and all diagnostic output - every log line, `info` and `debug` included, together with the interactive prompts (the identity question and the confirmations) - goes to `stderr`. This keeps a piped or redirected result clean. `alcove accept --identity IDENTITY URL INVITATION 2>/dev/null > matched.csv` writes only the matched-records CSV to `matched.csv`, with the invitation-terms display, the "wrote key file" line, the runtime banner, and every other diagnostic sent to `stderr`, where the same run without the redirect still shows them on the terminal. The result on `stdout` is an exchange's CSV output (when no `OUTPUT_FILE` positional is given), the invitation token printed by `invite`, the fingerprint value printed by `fingerprint` -- whose action banner, bound identity, `--force` regeneration warning, and out-of-band sharing instructions are diagnostics on `stderr`, so `FP=$(alcove fingerprint)` captures just the value -- and the verification verdict printed by `verify-receipt` (its exit code, nonzero only on a definite failure, reports the same result for scripts).

`--log-level <level>` selects the verbosity: `silent`, `error`, `warn`, `info` (the default), `debug`, or `trace`, matched without regard to case. `silent` suppresses all log output. Any other value is a usage error and exits 64, naming the value you gave, on every command that accepts the flag - a typo never quietly leaves the run at the default.

- **What the level governs.** It governs every diagnostic the run emits once logging is set up, including the low-level warnings from data cleaning and file handling, so `silent` keeps them out of the terminal and out of a `--log-file` alike and `debug`/`trace` turn up the rest of the run's detail - except a usage error in the command line itself (reported on `stderr` before either flag takes effect) and the last-resort line for an error no command handler caught (written straight to `stderr` at exit 1); neither is ever routed to a `--log-file`.
- **What it does not suppress.** It does not suppress the linkage terms `accept` shows when it stops to ask you to confirm them, nor the line saying what became of that question: those accompany the question rather than the log, and reach the terminal whatever this is set to, in the same rendering at every level (see [Offline acceptance](#offline-acceptance)).
- **The SSH handshake trace.** `--log-level trace` adds the SSH stack's diagnostics for an `sftp` connection to the diagnostic output, under the context name `ssh`: the server's identification string, the algorithms each side offered, the ones negotiated, and the packet flow, so one run answers whether the handshake completed and on what. It is roughly what `ssh -vvv` reports, without installing a second client to get it, and it is emitted at no other level. On a connection that completes it continues at one line per SSH packet, so a long transfer or a chatty remote side at `trace` emits thousands of them, with nothing bounding the count but a per-line length cap: give the run a `--log-file` rather than reading it in the terminal. The server's own text in those lines is escaped before it reaches your terminal or a `--log-file`, so a server cannot forge a log line with it.

`--log-file <path>` appends log output to `<path>` instead of writing it to the terminal, so Alcove can be run unattended or in automation without shell redirection. The file is opened in append mode, preserving any content from previous runs; each line already has an ISO-8601 timestamp, so successive runs stay distinguishable without a separate flag. The parent directory must already exist - a missing directory aborts the command with a usage error (exit 64) before any exchange work begins.

- **File permissions.** A log file Alcove creates is owner-only (mode `0600`), since at `debug`/`trace` it can record partner identity, linkage keys, and data categories; if you point `--log-file` at a file that already exists, its permissions are left as they are.
- **macOS extended ACLs.** A file also has an extended ACL, which those permissions do not describe and a file inherits from the directory it is created in, so Alcove clears it before the first line is written - on a file it creates and on one you point the flag at alike. A file whose extended ACL cannot be cleared is refused: the command stops with a usage error (exit 64) before any exchange work begins and writes no line into it. Inspect an extended ACL with `ls -le <file>` and clear one yourself with `chmod -N <file>`.
- **What is captured.** `--log-level` still applies to the file, so `--log-level silent --log-file run.log` writes nothing. Every diagnostic line is captured, including the low-level warnings from data cleaning and file handling. `alcove doctor`'s check lines are captured too, but as the plain rendering an operator reads rather than as timestamped log records (see [Checking a network file drop](#checking-a-network-file-drop)). A diagnostic Alcove composes as a block of several lines - the accept-time refusal listing what your configuration and the invitation disagree on, above the step that resolves it - keeps those line breaks in the file too, so the timestamp stands on the block's first line and the lines under it hold the block's own text.
- **What is not captured.** Results written to `stdout` (an exchange's CSV output, the fingerprint value) are not log output and are unaffected by `--log-file`.

For unattended runs, set `--peer-timeout` to a value that suits how long you are willing to wait for a partner that never appears (it defaults to one hour on `sftp` and `filedrop`; on `webrtc` an unset field leaves the transport's own defaults instead, ten minutes to meet the partner at the rendezvous then one hour of peer silence on the open channel); a partner that never appears at all makes the command wait out this budget at the rendezvous and live-exchange steps before exiting. The teardown after a successful exchange does not inherit this budget - it is bounded separately and short - so the long wait only applies while the exchange is still in progress. Wrapping the command in your pipeline's own outer timeout is still recommended as a safety check.
...
`--event-stream` emits a machine-readable event stream for a supervising process (an orchestrator, a job runner, a test harness) that spawns Alcove and needs structured progress and outcome events rather than parsed log lines. It is available on every exchange-running command - the zero-setup exchange, `alcove exchange`, and the online `alcove invite`/`accept` - and is off by default; it has no effect on an offline `invite`/`accept`, which runs no exchange, except an [acceptance that runs the exchange itself](#accepting-and-running-a-webrtc-exchange): given an `INPUT_FILE`, that offline-form accept emits the stream exactly as the online form does.

The stream is newline-delimited JSON on **file descriptor 3**, one event object per line, flushed as it happens: a stage-list event up front, a stage event at each protocol step and a stage-end event with that stage's duration when it completes, a warning event per non-fatal warning (a terms-exchange warning, a cross-party host-key divergence, or any local write a completed run was asked to make and could not - an audit artifact, or a configuration or consent record of an online `invite`/`accept`), a one-shot metrics summary, and exactly one terminal event - a result on success or a classified error on failure.

- **The warning event** names the notice that raised it in a machine-readable `source` field beside the message - the cross-party host-key divergence security signal, a per-run notice such as the resolved cardinality, or a local write a completed run lost - so a supervisor alerts on the ones it cares about without parsing prose. The values are listed in [docs/spec/CLI_EVENTS.md](spec/CLI_EVENTS.md#warning-sources).
- **The metrics event**, emitted just before the terminal event, reports the run's operational counters: how many records this party processed, how many times a transport data operation retried, and how many times the connection was re-established. Together with the per-stage durations this lets a supervisor - or an operator debugging a slow or flaky recurring exchange - see where the wall-clock went and how much the transport struggled.
- **The result event** reports whether a matched result file was written and, on a count-only run, the count -- and states what the two parties' agreed `deduplicate` values resolved to: your own value, the one your partner presented, and the cardinality the pair gives your side. The log states the same three at `info`, so this is where a supervisor running quieter, or reading fd 3 alone, reads them.
- **The error event** names one of four categories (`exchange`, `output`, `security`, `config`), a machine-readable abort reason distinct from the human log line, so a supervisor can tell a trust-boundary failure from a retryable transport fault. An authentication failure also has its own exit code (77); the other `security` failures -- a frame that fails its integrity check, a partner receipt that does not verify -- exit 69 like a transport fault, so only the category singles them out.
- **A signal interruption.** A run interrupted by a signal exits 130/143 without a terminal event (and so without a metrics event), which together is the interrupt signal. A signal arriving after the outcome was reported, while the transport is still closing, is the one case that pairs a terminal event with 130/143: the outcome stands and only the close was cut short.
- **The terminal event is the last line.** The transport is closed after it, and nothing that close raises reaches this stream; a supervisor that stops reading at the outcome misses nothing. An overrunning close is reported on stderr instead.

`stdout` and `stderr` are unchanged by the flag: the CSV result still goes to stdout and every diagnostic still goes to stderr, so the event stream never corrupts either. Wire fd 3 to a pipe when you spawn Alcove (for example, in Node, `stdio: ["inherit", "pipe", "pipe", "pipe"]`); if you pass `--event-stream` without wiring fd 3, the command fails fast with a usage error (exit 64) before any exchange work. The full contract - the framing, the per-line schema version, every event's fields, and the category rules - is in [docs/spec/CLI_EVENTS.md](spec/CLI_EVENTS.md).

## Exit codes

Every `alcove` command exits with one of the following codes. The failure classes 64, 65, 66, 69, 70, 73, and 77 follow the BSD `sysexits` convention.
...
| 65 | `EX_DATAERR` | `alcove verify-receipt` only: the verdict is a definite verification failure -- a commitment, signature, certificate binding, identity, terms hash, or receipt-record pairing check did not match. The run itself completed; this is not a crash. See [Verifying a receipt](#verifying-a-receipt). |
| 66 | `EX_NOINPUT` | `alcove verify-receipt` only: the verdict is incomplete -- nothing contradicted the record, but a check could not run because an input it needs (the re-supplied data, terms, a pinned fingerprint, your signing identity, or the exchange record the receipt pairs with) was not supplied or could not be read. Supply what the report names as not checked and run it again. A combined run that also found a definite failure exits 65. |
| 69 | `EX_UNAVAILABLE` | Usually a transport or availability failure: the exchange server, peer, or shared storage was unreachable, rejected an operation, or went silent, and retrying once the transport recovers may succeed. From `alcove doctor`, it means something the checks themselves depend on was not available, so nothing was established either way. It is also the code for the one failure that is neither: a completed exchange whose result table could not be built because the partner's payload did not fit the agreed shape. Nothing local failed to write there, so it is not a 73 -- but the exchange did happen, so read the terminal event's `output` category before retrying (see [Machine-readable event stream](#machine-readable-event-stream)). |
| 70 | `EX_SOFTWARE` | An internal fault in Alcove itself: a check on its own state failed, such as two of the run's own derivations of the same quantity disagreeing on inputs it had already accepted. Nothing in your configuration or your data is what to change, and a retry reaches the same refusal while conducting another exchange. **Do not retry; report it** with the error message, which ends by saying so. |
...
| 77 | `EX_NOPERM` | Authentication failed: the key exchange rejected the shared secret or the peer, the rotated shared secret could not be saved to the key file, or the SFTP server presented a host key other than the pinned one. A retry reaches the same refusal. **Do not retry.** For the shared secret, re-invite (see [Out-of-sync tokens](#out-of-sync-tokens)); for the host key, verify the server's key out-of-band and re-pin (see [SFTP host-key trust](#sftp-host-key-trust)). Treat an unexpected one as a possible security event. |
| 78 | `EX_CONFIG` | `alcove doctor` only: the checks ran and something they found needs changing before an exchange will work (see [Checking a network file drop](#checking-a-network-file-drop)). |
...
| 1 | unexpected error | A last-resort code for an error that escaped every command handler; ordinary faults use 64, 69, 70, 73, or 77 above. |

64, 65, 66, 69, 70, 73, and 77 are what the run itself reports: a `UsageError` and a connection misuse both map to 64, including one the transport reports wrapped as a transport failure, a definite `verify-receipt` verdict failure to 65 and an incomplete verdict to 66, an internal-consistency fault to 70, an authentication failure to 77, likewise when wrapped as a transport failure, and any other failure to the error's own exit code or 69 at the command error boundaries, while 73 is set where a completed run loses a local write. 78 is `alcove doctor`'s verdict code and is set nowhere else; 130 and 143 are set by the exchange's own signal handlers; 1 is the top-level catch-all. When `--event-stream` is active, a `security`-category failure is either an authentication failure, which exits 77, or another trust-boundary failure -- a frame that fails its integrity check on the authenticated channel, or a partner receipt that does not verify -- which exits 69 like a transport failure, so read the terminal event's category to tell that one apart (see [Machine-readable event stream](#machine-readable-event-stream)).
...
- **The agreed terms name something this build cannot run.** A linkage-key element transform naming a standardization function this build does not recognize, or a linkage algorithm other than `psi` or `psi-c`. The algorithm case is decided from the agreed pair once the terms are exchanged, so a run refuses here even where this party's own copy passed on its own. The transform case fires as that key's first row is read -- whether or not any row has a value for the element declaring it -- because the fault is in the terms rather than in the row that happened to reach it. Agree terms this build can run, or move both parties to a version that includes what they name.
...
- **An online `alcove invite` or `alcove accept` could not write its configuration.** What is missing is the configuration a later recurring `alcove exchange` reads. The rotated key is on disk, so recovery is to recreate the configuration -- not to re-invite -- and the error naming the cause was logged when the write failed.
- **A zero-setup `--save` run could not write the configuration, or the key file that accompanies it when both parties saved.** The exchange itself completed and its result is written; what is missing is the recurring-exchange setup. Fix what blocked the write, or point `--config-file` / `--key-file` elsewhere, and establish the recurring exchange with `alcove invite` -- re-running the zero-setup command would conduct a second exchange. The report names what became of each file, so a configuration that was written and could not be removed after the key file failed is named as still on disk; move or remove it first, or `alcove invite` refuses to overwrite it. A configuration file that appeared at the target path after the run's up-front check is reported the same way: the run refuses to overwrite it and reports the save as lost.
- **An online `alcove accept` could not refresh a consent record on a configuration it reused**, or **an online `alcove invite` could not record the payload columns it observed** into the configuration it had just written. The configuration itself stands; what is missing is a machine-managed record the next recurring exchange would have been held to, so that run reconciles against the older record, or lazily where there is none. The logged warning names which.
- **The result was not delivered, so what the run would have saved after it never ran.** A `--save` run's configuration and key file, or the payload columns an online `invite` records, come after the result reaches its destination; when that result is lost -- a stdout reader that stopped taking it, or a file that did not reach disk -- none of them is written. The run reports the skipped step beside the lost result, because your partner may have saved a recurring exchange this side did not: confirm with them what each side saved before setting one up, and do not re-run the exchange to get there.

A supervisor reads the response off the exit code alone: never retry a 73 -- a retried `alcove exchange` simply conducts another exchange with the rotated token, re-sending this party's data and waiting on the partner at the rendezvous again, as does a retried zero-setup run (a retried online `invite` or `accept` fails against the key file already on disk instead). What failed is named on stderr, and on fd 3 when `--event-stream` is active: a lost result is the terminal `error` event with `category: "output"`, the persistence that loss skipped is a `warning` event before it (the same warning precedes the exit-69 `output` error of a result table that could not be built), and every other condition above is a `warning` event beside a terminal `result` (see [Terminal-event guarantees](spec/CLI_EVENTS.md#terminal-event-guarantees)). The distinct code exists for the unattended run, where that stderr line is discarded by a supervisor or suppressed by `--log-level error`.

Cap how many times a supervisor retries automatically, and stop rather than loop on a retryable code without limit. The bound is about the count of attempts, not about diagnosing any one fault: every attempt re-runs the exchange protocol against the same set from the start, and 69 can mean the exchange already happened -- the completed-exchange, `output`-category case above -- so a loop that keeps retrying past a few attempts risks repeating a run rather than recovering one interrupted attempt. Repeated exchanges against the same set are not what the protocol's privacy guarantee is scoped to (see [Threat model](SECURITY_DESIGN.md#threat-model)): it assumes a partnership deciding when a run happens, not an unattended loop deciding how many times it happens. Stop after a small, fixed number of attempts -- three, say -- and page an operator instead of retrying further. A 70 is outside that budget entirely, as a 73, a 77, and a 64 are: each is deterministic in the run's own inputs, so no attempt after the first ends differently (see [The internal-fault code](spec/CLI_EVENTS.md#the-internal-fault-code) and [Exit 64 mid-run](#exit-64-mid-run-a-refusal-the-next-attempt-repeats)).

For `alcove exchange`, a missing, malformed, or unreadable configuration file (`alcove.yaml`) or key file (`.alcove.key`) - including a key file whose stored token is malformed - is a usage error and exits 64.

- **An unsupported channel or URL scheme** - a `ws://` URL on a path that cannot use one (the zero-setup and acceptance paths), a `ws://` URL holding more than the coordination server's location on the path that can, an `http://` or `https://` address to `alcove invite` naming more than the web app's own address, a `webrtc` connection with no `role`, a `webrtc` coordination-server address that forms no dialable WebSocket URL, an unknown scheme, or a malformed `file://` authority - is likewise a usage error and exits 64, as is a URL holding a malformed percent-escape such as a lone `%` (with any credential redacted from the message) or an invalid connection option or combination (for example a negative, fractional, non-numeric, or above-ceiling `--max-reconnect-attempts`, a non-numeric or out-of-range (outside `0..65535`) `--server-port`, a reserved `peer_id`, or a `retain_files`/`lockless_rendezvous` contradiction).
- **Failures during the exchange itself** - connecting to the server, the rendezvous, or the message loop - exit 69, unless the failure is the internal-fault condition above, which exits 70, an authentication failure, which exits 77, or one of the mid-run conditions that exit 64 (see [Exit 64 mid-run](#exit-64-mid-run-a-refusal-the-next-attempt-repeats)).
- **A successful run** exits 0; a run terminated by a signal exits 130 (SIGINT) or 143 (SIGTERM).

Passing a single-value option more than once - for example `alcove invite --accept-timeout 60s --accept-timeout 120s`, or a repeated `--log-level`, `--log-file`, `--server-port`, `--peer-timeout`, or `--linkage-strategy` - is a usage error and exits 64, naming the flag (`--<flag> may be given only once`), rather than silently taking one of the values. Count flags (`-v`/`--verbose`) and boolean flags (and their `--no-` forms, such as `--record`/`--no-record`) may still be repeated and keep their accumulate / last-one-wins / negation semantics.

Passing an unrecognized option - a misspelling such as `--server-user` for `--server-username`, or `--retain-file` for `--retain-files` - is a usage error and exits 64, naming the offending option, before any connection is attempted or file is written, on every command. It catches a mistyped credential or path override that would otherwise be dropped silently, leaving the run on the option's default or a stale configuration value. Positional arguments - the server URL, the input and output files, and the invitation string - are validated by each command, not by this check. The `invite`, `accept`, `init`, and `apply` commands accept a positional that may begin with `-` (an invitation string, or `-` for stdin; see [Invitation strings](#invitation-strings)), so on those commands the unknown-option check rejects only a `--`-prefixed token, which a positional never is, and leaves a single-`-` positional for the command's own validation.
