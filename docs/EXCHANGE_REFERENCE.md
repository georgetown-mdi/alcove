...
This document is the complete field-level reference for alcove exchange specifications. It covers all fields in the four components - linkage terms, connection, metadata, and data standardization - including types, valid values, consistency rules, and examples. It does not cover how the PSI protocol uses these parameters (see [PROTOCOL.md](spec/PROTOCOL.md)), the threat model or authentication design (see [SECURITY_DESIGN.md](SECURITY_DESIGN.md)), or the CLI commands that consume this file (see [CLI.md](CLI.md)). Intended for anyone configuring an exchange.
...
Exchange specifications are JSON or YAML documents that fully describe an Alcove exchange between two parties. All three Alcove applications read the same one: the web application provides an interactive editor for creating them, the console authors and runs one on the machine it is mounted on, and the CLI accepts them as configuration files. A file the web app mints for download is an ordinary `alcove.yaml` validated against this same shared schema -- there is no web-specific format and no secret in the file.
...
Because those three are read out of a record long after the run, they refuse a second class the `exclude` value does not: the nine Unicode text-direction formatting characters -- the embeddings, overrides, and isolates of Unicode UAX #9 -- which the [name rule](#linkage-terms) below refuses for the same reason. One of them opens a layout scope that outlives the value and reorders the copy the record is read beside, and nothing legitimate in an identity, a purpose, or a description needs one: a right-to-left sentence lays out correctly from its own letters. The implicit direction marks `U+200E`, `U+200F`, and `U+061C` are outside the class and stay admitted, in these three fields as in a name. An `exclude` value keeps the nine, being a value your data is matched against rather than text a record holds.

The document's names take a rule of their own, inside their 256-character cap: a linkage field, key, or element `name`, an element's `field` or `swap` reference, a [transform step's](#transform-steps) `function` name and each of its `params` keys, a [payload column's](#linkage_termspayload) `name`, the legal agreement's [`reference`](#linkage_termslegal_agreement), and a [rule set's](#linkage_termslinkage_rule_set) `name` may hold neither a control character nor one of the nine Unicode text-direction formatting characters -- the embeddings, overrides, and isolates of Unicode UAX #9. A name is matched on, written verbatim into both parties' exchange records, and shown beside first-party copy on the screen where the exchange is agreed to, and a text-direction character in one reorders the sentence around it. Nothing legitimate in a name needs one: a right-to-left or mixed-direction label lays out correctly from its own letters, and letters in any script are untouched. Alcove removes those same characters from your own CSV column headers when it reads the file (see [CSV header sanitation](spec/CHANNEL_SECURITY.md#csv-header-sanitation-at-ingestion)), so a header and a terms name agree on what a column may be called. The [`metadata`](#input-metadata) block's column `name` takes the same rule, for the same reason: a name you declare there is matched on, and a disclosed one is listed to your partner in the invitation.

Two kinds of value stay outside both rules, held to their length caps alone: the string values of a transform step's `params` -- a regex pattern, a replacement, a delimiter, a default -- bounded at 1000 characters each, and a name constraint's [`allowed_characters`](#constraints) class, bounded at 256. Each is data a step matches or substitutes with rather than a name, so a tab is a plausible delimiter and a line break a plausible replacement, and refusing them would refuse legitimate terms; a `params` key takes the name rule, being the parameter's name rather than data the step works with. What neutralizes a control character in one of those is the display escaping Alcove applies wherever it renders a value, the same boundary that covers every other partner-supplied string on its way to a screen, a log, or an error.

### The built-in rules

Not every exchange authors its own linkage fields and keys. A zero-setup exchange, an invitation minted on the web app's quick path, and the template `alcove init` writes all fill them in for you, from the product's built-in rules. They are also where the web app's Advanced invite path starts, before you edit the keys.

The built-in rules are two named artifacts, versioned separately:

- **`baseline-pii`, version `1.0.0`** -- the linkage **fields**: five PII elements, `ssn`, `ssn4`, `first_name`, `last_name`, and `date_of_birth`, each in its standardized form and with the constraints it holds. This is the substrate keys are built from, and it is general-purpose: it says which PII the built-in rules work from and how each element is cleaned, not what constitutes a match.
...
One string is not a label: the placeholder [`alcove init`](CLI.md#initialization) writes into a fresh template. A command that reads a label refuses that exact value -- alone in the field or on `--identity`, whitespace around it or not -- as firmly as it refuses none at all (see [Configuration](CLI.md#configuration)).

Omit the field to run unnamed. Alcove supplies no label of its own -- not the account it runs as, not a placeholder -- so terms holding no identity send none, the record omits its field rather than writing a stand-in, and every surface that shows a party name shows an absence marker in its place. Present, the value must be non-empty and within the length cap; an empty string is refused rather than read as an omission. The two commands that author a durable partnership, [`alcove invite`](CLI.md#offline-invitation) and [`alcove accept`](CLI.md#offline-acceptance), require a name at their own interface, so a configuration written by an acceptance always holds one. An exchange that signs receipts requires one too, and is refused before it runs without it: the certificate a party presents is trusted by the identity it used in the agreed terms, so an unnamed party leaves its partner nothing to check that certificate against (see [`signing.mode`](#signingmode)).

Under `certificate` signing mode this otherwise-free-text field is effectively pinned: it must match the identity bound into the party's signing certificate, or the partner rejects the receipt. A signed receipt therefore requires both parties to be named -- a certificate is trusted by the identity its holder used in the agreed terms, and an unnamed party has none for it to be authorized against -- so a run with signing configured refuses at terms agreement, before any linkage key or data crosses, rather than producing a receipt nobody can verify. The two commands that resolve an identity for an exchange report a mismatch locally rather than leaving it to the partner's exchange: [`alcove fingerprint`](CLI.md#signing-identity-fingerprint) warns, whether it binds the certificate or only reads an existing one, and [`alcove exchange`](CLI.md#signing-identity-and-the-agreed-terms) refuses the run before any credential, terms, or data are sent -- an exchange under a divergent pair cannot leave both parties holding a verifiable receipt. See [Signing](#signing).
...
Both strategies match on the several values a `split_on` produces, and on the several a `generate_fuzzy_comparisons` or a key declaring `swap` produces, reaching the same table on the same rows (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). What refuses those terms is the count-only algorithm, whatever the strategy. A both-sided `deduplicate` matches them under either strategy, grouping the records a key's several values reach (see [Deduplicating matches](#linkage_termsdeduplicate)).
...
> **Both parties `true` runs under either strategy:** `true` on BOTH parties resolves to many-to-many, where each party's records may group the other's, and both [linkage strategies](#linkage_termslinkage_strategy) match that pair over the same rows. A many-to-one match, `deduplicate: true` on one party and `false` on the other, runs under either too. No single party's terms hold the combination -- it takes both parties' `true` to resolve it -- so the pair is settled at the terms exchange, after each party's declared record count has been disclosed and before any linkage-key or payload data moves. Under `single-pass` one further thing follows from which party the exchange makes the receiver: that party resolves the whole pairing, so it is the one that reports the cluster diagnostic below, while its partner is handed the matched pairs alone.
>
> **A candidate set runs under a both-sided match:** terms that match a record on several values -- a [`split_on`](#fan-out-multi-value-fields) transform, a [`generate_fuzzy_comparisons`](#element-fields) expansion, or a key declaring [`swap`](#swapped-keys) -- change nothing about which strategy pairs the both-sided match. Under either the pair runs over those candidates, and the clusters it produces may join records that matched on different values of one key ([the entity closure](spec/PROTOCOL.md#the-many-to-many-entity-closure)). [The built-in rules](#the-built-in-rules) declare a swapped key, so a both-sided match over the shipped default keys is one either strategy runs.
>
> **What a both-sided match hands you:** the result file holds one row per matched pair under every cardinality, and where both parties group, those pairs resolve into [entity clusters](spec/PROTOCOL.md#the-many-to-many-entity-closure) -- a set of your records together with the partner records they link to. A cluster formed on one matched value writes `m * n` rows for `m` of your records and `n` of your partner's, and one joined together through a record that matched on several values of a key writes fewer -- one row per pair that matched. No column marks a cluster's boundary, and a reader that wants clusters computes them from the pairs. Both parties compute the same clusters from the one table, with nothing further exchanged for it. Which keys are worth matching on under that reading is [below](#choosing-linkage-keys-for-a-both-sided-match).
>
> **The run names what the pair resolved to:** nothing before the terms exchange can, since each party's value comes from its own file and the cardinality follows from the two together. So once the terms are agreed you are told your own `deduplicate` value, the one your partner presented, and the cardinality the pair resolved to -- on the CLI's log at `info`, its default level, beside the resolved role; on a browser seat's running screen at that same point and again on its completion panel; and on the console's completion panel, read out of the run it is watching. Your [exchange record](spec/EXCHANGE_RECORD.md#the-resolved-matching) holds the same three on every run whatever the log level, so an accounting read later names your partner's value and not only the resolved multiplicity. A run resolving to a deduplicating cardinality additionally names what that means for the result, raised more loudly -- on the CLI's machine-interface warning stream, and among a browser seat's run notices, shown beside an attended run and written to the diagnostic log by an unattended scheduled one. What that notice says about the result follows the agreed [`output`](#linkage_termsoutput) as well as the cardinality: a party those terms give no result is told the pairs land with its partner rather than in a file of its own. A both-sided run whose two declared record counts project a pair table past the advisory bound adds an advisory naming that projection and what each side contributes to it, so you can tell which side drives it; the bound and its derivation are in [PROTOCOL.md](spec/PROTOCOL.md#deriving-one-table-from-the-exchanged-association-maps). Neither notice refuses anything and the run continues; the projection is the largest table those counts admit rather than a prediction of the match.
>
> **Each party's own value, however the exchange is set up:** an invitation holds the inviting party's `deduplicate` and nothing more.
>
> - **Accepting an invitation:** where the invitation's shape admits a value for the accepting party's own side -- any shape but count-only, whose schema takes no `deduplicate` from either party -- the web accept screen offers that party a control for it, defaulting to `false`, and the value it holds reaches the terms that party presents at the run; the consent display states both parties' values and what the pair discloses. `alcove accept` offers no such control: on that route the accepting party's own value is always derived as `false`, so an accepted deduplicating invitation runs many-to-one with the inviting party as the "many" side.
> - **A zero-setup exchange** has no invitation and no accepting party: each side infers its own terms from its own file and declares its own value with [`--deduplicate`](CLI.md#zero-setup-exchange), off by default. Neither command line reads the other's, so the pair is settled at the terms exchange. The console's Direct exchange offers the same value as a control on its confirm step and states what the setting discloses over the terms it previews; the value reaches the recurring-run hand-off's command line as `--deduplicate`.
> - **The inviting party's own side** is held to what its invitation declared: an acceptance retains that value, and a partner presenting a different one when the exchange runs is refused before any key or payload moves.
> - **Two parties that each authored their own configuration file** have no such declaration between them -- the values differ by design there, which is what makes one of them the "many" side -- so that pair runs as written.
> - **An accepting party on a route with no such control** -- `alcove accept`, or a web accept whose invitation admits none -- groups its own records by declaring it in its own configuration file instead: each party's value comes from its own file, and both run `alcove exchange`. Where both files declare it, the exchange is the both-sided one above.
> - **That binding runs one way only:** nothing records an inviting party's expectation of its partner's value, so a party that accepted a deduplicating invitation and later declares `deduplicate: true` in its own configuration file runs the both-sided match, which the inviting party sees when the two parties' terms meet at the run.
...
To tell a good grouping from a bad one after the fact, a both-sided run reports what it grouped your result into: how many clusters, over how many records of yours and your partner's, and for each cluster size the number of distinct matched values that formed it. `alcove exchange` states it on the report that follows the run, and the browser states it on the completion panel. A run whose clusters are almost all one of your records against one of your partner's matched close to one-to-one; a cluster far larger than the rest formed on a single value is a key that named a group rather than a person, and one formed on several values was chained together by a record that matched on more than one value of a key. Both figures are your own run's: the cluster sizes are read off the table you received, and the value count off the matched values the run formed each cluster on, which the result file does not hold. Nothing is asked of your partner for either, and nothing refuses on them. Under [`single-pass`](#linkage_termslinkage_strategy) the report reaches the party the exchange made the receiver, which is the one that resolved the pairing; its partner holds the same pairs and can group them from the result file, but not the value count, which is read from matching state only the resolving party held.
...
A top-level key of `alcove.yaml`, a sibling of `linkage_terms` rather than a member of it: the [`deduplicate`](#linkage_termsdeduplicate) an accepted invitation declared for the *inviting* party's own side. It is what makes the paragraph above hold for a configuration rather than only for the moment of acceptance -- an accepting party keeps the declaration here, and an exchange run from this file refuses a partner presenting any other value at the terms exchange, before any key or payload moves. Every accept path records it: `alcove accept`, online and offline, and the browser's managed and console flows.

Do not confuse it with `linkage_terms.deduplicate` beside it, which is *your own* side. Accepting takes your own value from your own seat's control where it offers one, `false` otherwise, rather than from the invitation, so the two legitimately differ regardless, and the run reads the binding only from this key.

Omit it for an exchange you and your partner each authored from your own configuration files: there is no declaration between you, the differing pair is what makes one of you the "many" side, and an absent key holds the partner to nothing. Do not add one by hand to make a partner's setting "official" -- it records what an invitation stated, and a value you chose yourself would abort a correct exchange.
...
| `generate_fuzzy_comparisons` | string | no | Method for generating additional values for fuzzy matching: `transpositions` generates every two-position transposition of the value, all pairs of positions and not adjacent ones alone; `edit_distances` generates all single-character deletions, matching values within one edit distance; `adjacent_years` generates dates +/- 1 year from the input; `day_month_swaps` generates the date read with its day and month exchanged, where that reading is a real calendar date. Applied after any transformation. An element declaring one costs the candidate values its expansion can reach toward the [`single-pass`](#linkage_termslinkage_strategy) dataset ceiling, in place of 1, and that count follows the width the element's own `transform` bounds its value to: a `substring` to 10 characters costs 11 for `edit_distances` and 46 for `transpositions`, while an element whose transforms bound no width costs what the full 128-character expansion limit reaches (129 for `edit_distances`). `adjacent_years` costs 3 and `day_month_swaps` 2, whatever the width. A fuzzy expansion on an element whose transform bounds no width exceeds the per-key ceiling on its own, with no second expansion needed to reach it. Bounding the element first with a transform that fixes its width -- a `substring` to a length, a `phonetic` code, a `parse_date` layout -- is what admits it, whether the key declares one such expansion or stacks several; the arithmetic is in [PROTOCOL.md](spec/PROTOCOL.md#the-width-bound-a-per-key-candidate-cap-the-terms-declare) |

**Bounding a `transpositions` element.** Its count is one candidate per pair of the value's positions, so it grows with the square of the width: 46 candidates at 10 characters. An element declaring `transpositions` must bound its value with a `transform` that fixes its width -- a `substring` to a length, a `phonetic` code, a `parse_date` layout. Terms whose element bounds none are refused before the exchange runs. The widest width a single element may declare, and the tighter bound two stacked elements must each meet, are derived in [PROTOCOL.md](spec/PROTOCOL.md#which-party-expands-full-variant-and-deletion-neighbourhood-expansions).

**The date kinds need a `parse_date` ahead of them.** `adjacent_years` and `day_month_swaps` read a value's year, month, and day by position, so an element declaring either one needs a `parse_date` transform emitting the default `YYYYMMDD` layout before it. The first row whose value is not eight digits -- `01/15/1990`, a name, a nine-digit identifier -- refuses the exchange rather than matching on the exact value alone, since matching a row on less than the terms declare would narrow what the partner's consent display states. A value that is eight digits but names no real calendar date, such as `19990229`, emits no candidate at all, so a pair matches the same way whichever party resolves as the receiving one.

**Which party expands.** `transpositions`, `adjacent_years`, and `day_month_swaps` enumerate the whole set of values one transposition, one year, or one day/month exchange away from the record's own, so one party enumerating is enough for a pair that far apart to match: they are generated by the receiving party alone, and the result is the same whichever party that turns out to be. `edit_distances` is generated by both parties, because two values a substitution apart meet only where each side's deletions overlap. Nothing about this is authored -- it follows from the method named -- and the reasoning is in [one-sided-fuzzy-expansion.md](notes/one-sided-fuzzy-expansion.md).
...
The named rule set the `linkage_fields` and `linkage_keys` above were drawn from -- a citation, not a specification. It names two artifacts and their content versions: `field_set` for the fields, `key_set` for the keys. [The built-in rules](#the-built-in-rules) are the one set Alcove ships.
...
Each `linkage_terms.payload.send` and `linkage_terms.payload.receive` list names a column once. If `send` or `receive` names the same column twice -- in your own file or in a partner's invitation -- Alcove reads it once: the first entry stands, with its description, and the repeat is dropped. So the dictionary shown on the consent screen and cross-checked against your partner's list names the column a single time, and a duplicate does not fail the exchange. The top-level `expected_payload_columns` and `disclosed_payload_columns` lists documented next, and the `outbound_payload_consent.columns` list beside them, are read the same way: a name repeated in any of them is kept once at config parse, so a typo there does not abort the run at payload reconciliation as a disclosure mismatch with your partner.
...
**`expected_payload_columns`, `disclosed_payload_columns`, and `outbound_payload_consent` are top-level keys of `alcove.yaml`, siblings of `linkage_terms`, not members of `payload` or of `linkage_terms`.** The level is critical rather than cosmetic: `linkage_terms` and `payload` both strip keys they do not recognize, so one of these three nested inside either is dropped silently at parse -- no error, no warning, and the disclosure enforcement it was meant to establish simply does not exist. Check the indentation when you author one by hand.

The three are per-party local records that are never exchanged, cross-checked against the partner, or folded into the agreed-terms hash (each is kept out of the mirror by design). What they do is hold the actual runtime disclosure to what was promised, or consented to, at setup:

- **`payload.send`** -- the columns this party discloses, exchanged with the partner as a data dictionary. It must agree exactly with what this party's [input metadata](#input-metadata) actually transmits (`is_payload: true` and `role` not `ignored`, the single source of truth for disclosure); a `send` that over- or under-declares that set is refused before the exchange runs, so the dictionary shown for consent and written into the [exchange record](spec/EXCHANGE_RECORD.md) matches the bytes that flow.
  - Empty is strict and absent is lazy, as for the three fields below: omitting `send` is exempt (the guided and default paths author none while metadata still transmits), while an explicit `send: []` declares that this party discloses nothing. It is held to that whenever the partner is entitled to the matched result.
  - Accepting an invitation that declares `receive: []` gives you exactly that `send: []`, because the two are mirrored -- so a partner that asks for nothing, paired with an input file whose extra columns default to transmitted, is refused locally, before any credential, terms, or data are sent, naming the columns. Mark them `is_payload: false` (or `role: ignored`), or ask for a corrected invitation.
  - A partner entitled to no result is sent no payload at all, so there the same pair runs and transmits nothing rather than being refused; a `send` that names columns is still held to the disclosed set in either direction, since that dictionary is exchanged, consented to, and recorded whatever moves.
- **`payload.receive`** -- what this party expects to be disclosed. Omitting it is lazy ("take whatever I am given") and that direction is not cross-checked; declaring it -- including an explicit empty `receive: []`, which asserts the partner discloses nothing -- is strict, and any divergence from the partner's `send` aborts the exchange before data moves. This empty-is-strict, absent-is-lazy convention lets an invitation be authored without the inviter knowing the partner's columns: the inviter declares only its `send`, the accepting party adopts that as its own `receive`, and the inviter takes whatever the partner's metadata turns out to disclose. Laziness relaxes only this declaration check, never what is disclosed -- each sender's own metadata always governs what it sends.
  - An explicit `receive: []` is held against what the partner's process actually discloses as well as against its declared `send`, wherever you are entitled to the result. Where the partner declares no `payload.send` at all -- the guided and default paths author none -- while its input metadata transmits a column, the two documents still mirror, but the run contradicts your declaration and is refused once both parties' terms are agreed, before any linkage round, association table, or payload moves, under either [linkage strategy](#linkage_termslinkage_strategy). Both parties are refused, and `alcove exchange` exits 69. Where you declared `expects_output: false` instead, your partner sends you no payload at all, so the empty `receive` the schema requires of you there contradicts nothing and the pair runs. To go ahead with those columns, have your partner declare them in its `payload.send` and declare them in your own `receive`, or omit `receive` to take whatever it sends; to go ahead with none, have your partner mark them `is_payload: false` (or `role: ignored`) so its metadata transmits none.
- **`expected_payload_columns`** (local, receive-side) -- the set this party locked in as what it will receive, checked against the *actually-received* columns at runtime after the payload exchange; a divergence aborts the exchange (the partner promised one disclosure and delivered another). It is set from the disclosed subset an invitation holds at accept time, or crystallized from a first observed exchange (an online inviter, or a `--save` run), and falls back to `payload.receive` for an authored recurring config. Same empty-is-strict, absent-is-lazy semantics.
...
- **`outbound_payload_consent`** (local, send-side) -- what an *accepting* party confirmed about its own outbound columns. An invitation authors what its sender discloses and, usually, nothing about what it will receive, so nothing in it authors the accepting party's own outbound set: that set comes from your input file, where a column Alcove does not recognize is transmitted by default. Accepting records what it showed you (`status: confirmed` with the columns, or `status: pending` where it had no input file to resolve them from), and a run that resolves a different set -- wider or narrower -- shows it and asks again before any credential, terms, or data are sent, or refuses (exit 64) where there is no terminal to ask on. Omit it for an exchange whose outbound columns you authored yourself, such as one you invited a partner to; a run then sends whatever your metadata discloses, as `disclosed_payload_columns` holds it to what you promised.

Each of the three lists column names, so each name takes the [name rule](#linkage-terms) beside its 256-character cap: a list holding a control or text-direction character is refused at config parse. `expected_payload_columns` is written from the disclosed subset an invitation holds, and the same shape is applied where that invitation is decoded, so a partner cannot mint a token that puts such a character into your configuration.
...
In practice you rarely author them: every path that establishes an exchange writes the ones it owns. Accepting an invitation records what the invitation disclosed and what the acceptance showed you; inviting records the subset the token published, and re-inviting refreshes it; a `--save` run crystallizes what the first exchange actually disclosed. Each is refreshed by the operation that could invalidate it, so the file cannot promise one disclosure while the exchange performs another. What you author by hand is the pair inside `payload`; what you review before an unattended run is these three.

The runtime enforcement mechanism, the wire field an invitation holds, the accepting party's own consent record, and how the observe-then-persist paths reach the same enforcement are specified in [EXCHANGE_FILE.md](spec/EXCHANGE_FILE.md#payload-disclosure-consent).
...
The smallest block each channel takes, to paste into `alcove.yaml` in place of the placeholder `alcove invite` and `alcove accept` write. Every field is described in the sections below; the tuning under [`connection.options`](#connectionoptions) is optional everywhere.
...
The peer addresses are derived from the shared secret, so there is nothing to fill in beyond the coordination server; [`connection.role`](#connectionrole) is `inviter` on the party that issued the invitation and `acceptor` on the party that accepted it. `alcove accept` seeds this block whole from the invitation -- the relay the invitation names included, as [`invitation_relay`](#connectioninvitation_relay) -- so it usually needs no edit at all.
...
**From the CLI.** Pass `--outbound-path DIR` on the zero-setup exchange, `alcove invite`, `alcove accept`, or `alcove exchange` to configure a split directory without hand-writing the pair. The directory the command already names becomes the inbound directory -- the server URL or positional path for the zero-setup, invite, and accept commands, or the loaded config's directory for `alcove exchange` -- and `--outbound-path` becomes the outbound. It applies to both `sftp` and `filedrop` and rejects an outbound equal to the inbound, under the same rules a hand-written config is validated against. It requires retain mode: pass `--retain-files` (which implies `--lockless-rendezvous` and `--timestamp-in-filename`), or, for `alcove exchange`, have `retain_files: true` in the config it reads. Each party sets its own `--outbound-path`; it is a per-party choice, not negotiated with the peer.
...
| `secure` | boolean | WebRTC only | Whether the signaling socket is opened over TLS (`wss:`) rather than plain `ws:`. Defaults to `true`; signaling sends the derived rendezvous ids and both parties' candidate addresses, so plaintext is a choice made by design for a server reached without a network in between (a loopback or test broker), never one an omission produces. A browser peer has no equivalent field: it takes the scheme from the page it was served over. The `port` default follows it -- 443 when secure, 80 otherwise -- and `path` defaults to `/` |
...
`alcove invite` and `alcove accept` set the field themselves on every WebRTC connection block they write -- `inviter` from the inviting side, `acceptor` from the accepting side -- so the two parties to one exchange hold complementary roles with neither operator authoring it. A web app's `http://`/`https://` address or a `ws://`/`wss://` URL to `alcove invite` builds the webrtc connection block, saves it, and mints an invitation carrying that same host/port/path as the endpoint the partner dials; `alcove accept` seeds its own connection block from that endpoint. See [Inviting over WebRTC](CLI.md#inviting-over-webrtc).
...
- **`alcove invite` over a `ws://` or `wss://` URL** takes the relay from `--turn` and `--stun` ([CLI.md](CLI.md#inviting-over-webrtc)), and the configuration it writes holds them as url-only `turn` entries and a `stun` list.
- **An offline `alcove invite` from a configuration whose connection is `webrtc`** names that connection's coordination server and its relay, from `turn` and `stun` as above. A configuration with neither names no relay.
...
|-------|------|---------|-------------|
| `peer_timeout_ms` | integer | 3600000 | Milliseconds to wait for the partner at any single step before giving up, applied both to the initial rendezvous and to each message exchanged during the protocol; if the partner goes silent past this window the exchange fails with a transport error. The effective limit is the minimum of this and the remaining shared-secret lifetime. Must be a positive integer of at most 604800000 (seven days), the same ceiling `--peer-timeout` holds. It does not govern teardown, which runs on a short fixed budget of its own that this value caps only if you set it lower -- and capping the wait does not shorten the close, so a very small value can leave the command printing its teardown line a little after its final output. See [SFTP connection teardown](#sftp-connection-teardown). The 3600000 default is the file-sync (`sftp`, `filedrop`) one. On `webrtc` an absent field is not that default: the transport's own three budgets apply -- 600000 at the rendezvous, 30000 for the data channel to open once both parties' session descriptions have been exchanged, and 3600000 of silence on the open channel -- and a value set here replaces all three. |
| `server_connect_timeout_ms` | integer | 30000 | Milliseconds to wait during each connection attempt to the primary exchange server. Must be a positive integer (zero is not a meaningful "no timeout"). On `sftp` an attempt has two phases and this value bounds each of them: the handshake and authentication, and then the request that opens the SFTP subsystem, which begins the moment authentication succeeds. So a stalled attempt ends within roughly twice this value plus a forced-close wait of at most one second, rather than at it. An attempt whose subsystem request goes unanswered for the whole window ends the connection terminally, without a further attempt, reporting the phase that timed out -- distinct from a rejected credential and from a handshake that never completed. It bounds an attempt rather than the dial, as `curl --connect-timeout` and OpenSSH's `ConnectTimeout` do: a server that accepts the connection and then answers nothing is re-dialed, so the whole wait is this value once per attempt plus a second between attempts -- `max_reconnect_attempts + 1` of them, about two minutes at both defaults. Set `max_reconnect_attempts` to 0 for a dial that spends this value once, which is what an unattended run wanting its stated budget to be the whole budget should set. Why it bounds an attempt, and what comparable tools do: [connect-timeout-prior-art.md](notes/connect-timeout-prior-art.md). |
| `max_reconnect_attempts` | integer | 3 | How many times a connection may be re-dialed after a fast transient failure, and on `sftp` how many sessions one exchange may lose mid-run -- one setting spending two separate tallies of the same size. What each bounds, how a lost session is charged, and the ceiling the value itself is capped at are in [CHANNEL_SECURITY.md](spec/CHANNEL_SECURITY.md#sftp-mid-exchange-session-recovery); what to set it to is [below](#setting-max_reconnect_attempts). |

#### Setting `max_reconnect_attempts`

**What the one number governs on each channel.** On `filedrop` it governs the initial connect alone -- the channel holds no session -- retrying a fast transient such as a share or its permissions still settling, each attempt bounded by `server_connect_timeout_ms`. On `sftp` the same number additionally caps how many sessions one exchange may lose mid-run: a clean session drop (typically a server-enforced session or idle limit) is re-dialed with the pinned host key and stored credentials and the interrupted operation re-issued, so the exchange survives it. The two tallies are separate counters and do not share one budget.

**What an exhausted budget looks like.** The exchange ends with a terminal non-zero error naming this budget and reporting the run's lost-session tally against it: every lost session spends one, whether its re-dial succeeded, failed, or was refused because the budget was already spent. The error names two remedies -- raise this setting for a flaky link, or set [`connection_per_poll`](#sftp-only-options) for a server that caps session lifetime. The mid-run tally is strictly cumulative and does not reset on progress, does not apply in `connection_per_poll` mode, and is not charged for the teardown abort-marker write.

**Zero is a valid value** and means what it says on both counters: connect once without a dialing retry, and treat the first mid-run drop as terminal. A value above the ceiling is refused with a usage error (exit 64) before anything is dialed, whether it came from `alcove.yaml` or from `--max-reconnect-attempts`.

**What no budget re-dials.** A key exchange that fails because the server accepts only algorithms this host's crypto provider cannot perform is terminal at the first attempt, while the same negotiation failure on a host missing nothing is retried like any transient (see [Key-exchange algorithms and the host's crypto provider](#key-exchange-algorithms-and-the-hosts-crypto-provider)). A fatal protocol error, a rejected host key, and the peer-timeout budget each end the exchange terminally whatever this setting holds. A timed-out attempt is not retried uniformly either: a `filedrop` connect that reaches its per-attempt timeout is terminal at that one window, as is an `sftp` dial whose SFTP-subsystem request goes unanswered, while a dial that times out in its handshake phase is retried like any other attempt ([CHANNEL_SECURITY.md](spec/CHANNEL_SECURITY.md#connect-probe-bound) states why each reading is what it is).
...
| `peer_id` | string | - | A stable, human-readable identifier for this party. Appears in every filename this party writes (hello, message, ack) and in server-side logs and transcripts. When unset, a UUID is generated at construction time. **Recommended for unattended and scheduled runs**, where it turns the leftover a killed run leaves into an immediate start-up refusal naming the file rather than a failed run (see [Directory exclusivity](#directory-exclusivity)); note that a stable id also makes this party's runs linkable to each other in the partner's logs. Requires `timestamp_in_filename: true`; a reused stable id without a timestamp segment can collide with a leftover file from a crashed prior session. The two parties must use distinct ids, and neither may be the other's id extended by `-` (e.g. `"site"` and `"site-2"` are rejected at rendezvous; see [FILE_SYNC.md preconditions](spec/FILE_SYNC.md#preconditions-for-a-correct-exchange)). Spaces and `-` are permitted within a `peer_id`. It is at most 211 bytes of UTF-8, and the two parties' ids together must keep every file name within 255 bytes. The value `"temp"` is reserved. Filesystem-unsafe characters (`/` and NUL on all platforms; `<`, `>`, `:`, `"`, `\`, `|`, `?`, `*` on Windows NTFS) are not validated but may cause errors at the transport layer. |
| `retain_files` | boolean | false | When `true`, no exchange file is deleted as a protocol step: the receiver writes an [acknowledgment marker](#acknowledgment-markers) for each message it consumes, the sender gates its next write on that marker rather than on the message file disappearing, and once rendezvous completes the shared directory is a permanent transcript. It is for a transport that does not propagate deletions, and for an auditable record of what flowed each way. The mode's mechanics and its bilateral detection are in [FILE_SYNC.md](spec/FILE_SYNC.md#two-orthogonal-mode-axes); what to set beside it, and what your partner is told, are [below](#setting-retain_files). |
| `unexpected_files` | enum | mode-coupled (see description) | How to handle a file that appears in the shared directory *during* the message loop and is neither recognized as part of this exchange nor an in-flight temporary write of its own -- a sign the directory is being shared with another process or session, or that a sync tool produced a conflict copy or partial download (see [Directory exclusivity](#directory-exclusivity)). One of `error` (fail with a usage error (exit 64) naming the file and the directory path), `warn` (log once per distinct file name and continue), or `ignore` (skip silently). **Local, not bilateral**: detecting a foreign file is an observation of one's own directory view, needs no peer agreement, and has none of the mismatch-stall risk of `lockless_rendezvous`/`retain_files`; the two parties may use different values. When unset the effective default is mode-coupled: `error` on plain delete-mode transports (ordinary `sftp`/`filedrop`) and `warn` when `retain_files` or `lockless_rendezvous` is set -- those flags signal a sync-mediated transport that legitimately produces transient conflict copies and partial downloads mid-session, where a hard fail would abort exactly the exchanges retain mode targets. An explicit value always overrides the mode-coupled default. This setting governs foreign-file detection only; a malformed *protocol* file (a peer-prefixed, message-shaped name a correctly configured peer cannot produce) is always reported regardless of this setting. |

#### Setting `retain_files`

**What has to be set beside it.** Retain mode requires `timestamp_in_filename: true` -- without it every message from the same party would share a filename and a retained transcript would overwrite itself -- and `lockless_rendezvous: true`, lock rendezvous being delete-based and unable to produce the whole-directory no-delete transcript retain mode guarantees. The CLI `--retain-files` flag sets both when they are not already set.

**Both parties set it, and a difference fails the run.** It is a bilateral agreement with no negotiation step: each party advertises its own value in its hello, and a difference fails fast with both sides named, exactly as `lockless_rendezvous` does ([FILE_SYNC.md](spec/FILE_SYNC.md#bilateral-configuration-detect-and-fail-never-negotiate)). When both flags differ at once -- only possible as `retain=true`/`lockless=true` against both `false`, since `retain_files` requires `lockless_rendezvous` -- the error names the `retain_files` mismatch, which a single re-run realigns.

**Your partner is told before they consent.** An invitation minted while retain mode is on declares it, and the accepting party is shown the retained transcript on its acceptance display -- so a partner learns what the exchange leaves behind rather than meeting it at a failed run. The declaration is disclosure only: it sets nothing on the accepting side, which still configures its own half here, and a mismatch still fails at the same place ([FILE_SYNC.md](spec/FILE_SYNC.md#retain-mode-declaration-on-the-token)).

**Each exchange needs a fresh directory**, enforced before the run: a stale message or ack marker from a prior session would corrupt or stall it. A rendezvous that fails before it completes is the exception and clears its own files. Reusing a retain directory is a CLI-only recovery by design; see [Directory exclusivity](#directory-exclusivity) for the guard, the flags that clear it, and what they discard.
...
Closing an SFTP connection is a two-party act: this side disconnects, and the partner's server closes the connection. Some servers accept the disconnect and then go quiet, leaving it half-open. Alcove bounds that wait in **both session modes**, the default held session and [`connection_per_poll`](#sftp-only-options): past a short fixed teardown bound it closes the connection from this side and the command finishes normally.
...
With `timestamp_in_filename: true`, the filename additionally holds a timestamp and counter:
...
Setting [`peer_id`](#connectionoptions) avoids all of this, which is why it is recommended for unattended and scheduled runs: the leftover then holds your own id and is refused at start-up on the filename alone. Do not reach for `--sweep-exchange-files` on a schedule instead -- it asserts no concurrent session is using the directory, which an unattended run is in no position to assert on every invocation. How the entrant distinguishes the two cases, and the bounded window it waits, are in [FILE_SYNC.md](spec/FILE_SYNC.md#phase-1----entry-present-peer-hello).
...
- **Runtime-injected secret state** (`shared_secret`, `expires`): loaded from the key file (`.alcove.key`) and added to the in-memory representation before the exchange runs. These never appear in `alcove.yaml` and must not be edited manually. If `shared_secret`, `sharedSecret`, or `expires` are present in the configuration file, the CLI emits a warning and ignores them; values from the key file always take precedence.
- **Operator-policy fields**: settable in `alcove.yaml`. `token_max_age_days` is the first (see the field table below). The loader leaves these for schema validation; the schema honors a recognized policy field and rejects an unrecognized key (see the unknown-key note below).

`shared_secret` is required for recurring exchanges run via the `exchange` command. If the key file (`.alcove.key`) is absent, the CLI aborts before any connection is attempted. Zero-setup exchanges (the default, subcommand-less invocation) rely on transport-layer authentication instead and do not use a key file.

**Provisioning the key file from an invitation.** When you compose an exchange in the web application, you download a configuration file that -- like every `alcove.yaml` -- never holds the secret. Pass `--invitation CODE` to `alcove exchange` to complete your local provisioning from the invitation code (the same code `alcove accept` takes) in the same command that runs the exchange: the CLI decodes and validates the code (checksum, schema, and expiry), writes your key file with the invitation's shared secret and expiry, and then runs the exchange normally. Keep the code out of shell history with the `@`-file form, `--invitation @code.txt`. A malformed or expired code fails before anything is written. `--invitation` is refused if a key file already exists at the key path: the secret rotates after your first exchange, so the original code can no longer establish a valid key -- run without `--invitation` to use the existing one, or re-invite to establish a new secret. The secret rides only the invitation code and never the downloaded file; the fail-closed provisioning ordering and the channel-binding rule an accepting tool honors are specified in [EXCHANGE_FILE.md](spec/EXCHANGE_FILE.md).
...
An operator-policy field, not key-file-injected. Rotation tokens written after each successful exchange have no expiration by default, on the assumption that active partnerships exchange frequently. A dormant partnership (a monthly cadence, a holiday gap) could otherwise hold a valid token indefinitely. When `token_max_age_days` is set, a successful exchange stamps `expires` = (the moment of rotation) + `token_max_age_days` days into the rotated `.alcove.key`, bounding a token's age independently of exchange frequency.

The stamp is enforced at the next `alcove exchange`, which reads the token's `expires` at load time:
...
The receipt signing mode. `none` signs no receipt (only the unsigned self-attested record is produced). `session-derived` is a MAC under the shared key-exchange session key -- tamper-evident but not non-repudiation and not third-party verifiable; it is not yet implemented, and a configuration selecting it is refused as a configuration error before the exchange runs rather than left to complete unsigned. `certificate` signs with this party's long-lived signing identity and is the only mode that yields third-party-verifiable non-repudiation; it requires a named party ([`linkage_terms.identity`](#linkage_termsidentity)), and a configuration omitting it is refused before the exchange runs for the same reason. It does not require [`signing.partner_fingerprint`](#signingpartner_fingerprint): a run with none on file pins the certificate the partner presents at the authenticated setup step. A configuration that can neither pin nor establish one -- certificate mode on a run with no authenticated setup step to present a certificate on -- is refused before it runs. Under `certificate` mode an authenticated CLI exchange produces a dual-signed receipt: both parties sign the same terms and data-flow facts and swap signatures over the connection, each verifying the partner's pinned certificate before its signature (see [PROTOCOL.md](spec/PROTOCOL.md#the-signed-receipt-step)). A stored dual-signed receipt is verified again later with [`alcove verify-receipt`](CLI.md#verifying-the-signed-record).
...
This is a local path, not an [`@`-file reference](CLI.md#configuration). A leading `~` (or `~/`) is expanded to the home directory, so an operator who names one is honoured exactly -- what Alcove does not do is choose the home directory itself.
...
*Required:* no; recorded by the first exchange when absent

The partner's pinned certificate fingerprint. A presented partner certificate is trusted only if its self-signature verifies and its fingerprint matches this value; a mismatched value rejects the partner's certificate (and therefore any receipt it holds) with a clear error, before any data crosses. The fingerprint is not secret, but the channel it arrives over must be authentic. It stays valid until the partner regenerates its identity.

The field has two ways of being filled, and they differ in what the pin is anchored to.

- **Set it yourself, before the first run.** Ask the partner for the value their `alcove fingerprint` prints, over a channel you trust, and write it here. The first exchange holds the certificate the partner presents to this value and refuses a partnership that does not match it. This is the stronger posture: the anchor is the channel you chose.
- **Leave it out and let the first exchange record it.** Both parties present their certificate during the authenticated setup step, so an exchange with no pin on file adopts the one presented, writes it into this file, and states the value it recorded on stderr and on the machine-readable warning stream. The anchor is then the channel the invitation and its shared secret travelled, and nothing else -- so compare the reported fingerprint with the partner out of band before relying on the receipts. See [SECURITY_DESIGN.md](SECURITY_DESIGN.md#pinned-self-signed-trust-model).

Only `alcove exchange` writes this field, only into the configuration file named on its own command line, and only when the file holds none: a run whose partner presents a different certificate is refused and leaves the file exactly as it stands. Changing a pin is a deliberate edit, as changing a host-key pin is.

No cross-field rule ties this value to `signing.mode` in the block's schema, so a partially-authored config -- no partner fingerprint yet -- still parses wherever the schema is used; `alcove fingerprint` needs only `identity_file`, which it reads from the raw config text rather than through the schema.
...
Optional. The field delimiter Alcove reads your input CSV by and writes your result file with.
...
With the key absent, Alcove reads the file with commas and writes the result with commas. A `--csv-delimiter` on a command reading this configuration replaces the key for that run, and `alcove init --csv-delimiter` writes the key into the template it produces; see [CLI.md](CLI.md#the-field-delimiter).
...
A column name that holds invisible control characters -- a tab, a NUL, an escape, or one of the text-direction characters among them -- loses them at every read (see [CSV header sanitation](spec/CHANNEL_SECURITY.md#csv-header-sanitation-at-ingestion)), and the seat that read the file states which column positions changed. Two consequences reach this block. A name made only of those characters comes back empty and meets the refusal above, which states the removal as the cause instead of the header-row causes. And a name written as the header was typed -- a `metadata` `name`, a `linkage_fields` `name`, or a `standardization` output -- does not name a column of the file: write it as the column without those characters, which is the name `alcove init` writes into a configuration it authors from that file.

A declared name holding those characters is refused rather than reconciled against the header. Where it is a `linkage_fields` name, an element `field` or `swap` reference, a `payload` column name, the legal agreement's `reference`, or a rule set's `name`, the name rule above refuses the document outright; where it is a `metadata` `name`, the same shape refuses it here, under a message naming this block. Either way the configuration fails to load, naming the field by its issue path and printing none of the value, and no exchange runs -- so no column name you declare reaches your partner with these characters in it, the invitation's disclosed payload column list included. The path is spelled as the command that read the file spells it: `alcove exchange` names `linkageTerms.linkageFields.0.name` for a terms name and `metadata.0.name` for one declared here, while `alcove invite` names `linkage_fields.0.name` under a prefix naming the linkage terms and a bare `0.name` under one naming the `metadata` block. A `standardization` `output` or `input` is outside the rule: it names a linkage field and a column of your own input, and neither is sent. Each is still held to the same 256-character ceiling every other declared name has, in a configuration file as at the console's own boundary.

A signed receipt or exchange record minted from such a header holds the column name as the mint recorded it, so verifying it against a read of the same file is the same shape and takes the same correction. That one is stated from the mechanism rather than measured.

The length ceiling on a column name is enforced on the inferred path too, but only where the name is actually transmitted: a column the metadata transmits (`is_payload: true` with a role other than `ignored`) whose name is longer than the ceiling is refused during data preparation, before any credential, terms, or data are sent.

- A party whose partner is entitled to no result (`output.share_with_partner: false`) transmits no column at all, so the bound does not apply to it.
- A payload column's name travels with its values -- the partner refuses a longer one when it parses the payload, and the exchange record cannot record it -- so the exchange could not complete. Shorten the header, or set the column not to transmit.
- A column used only for matching or ignored transmits its name nowhere, so an over-long header on one of those is no obstacle and the file is not refused over it.
- The record identifier is not exempt: an inferred identifier column keeps `is_payload: true` and is transmitted (see the identifier rows below), so its name is bounded like any other payload column unless the column is set not to transmit.
- The web app applies the same bound where the columns are marked to send, so an oversized name is refused before anything is transmitted rather than after.
- `alcove invite` applies it where it reads a disclosed set from an input file's header, naming the offending column positions and the ceiling as a usage error (exit 64) before the invitation is printed or a configuration or key file is written, so an invitation never publishes a column the exchange it commits to could not transmit.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | yes | Column name in the input CSV; must be non-empty, at most 256 characters, unique within the metadata block (matched case-sensitively), and free of control and text-direction characters (the [name rule](#linkage-terms) above). A violation is rejected at config parse |
...
standardization:
  ...
  - output: last_name_variants   # fan-out: one row -> multiple PSI entries (see Fan-out below)
    input: LAST_NAME
    steps:
      - function: to_upper_case
      - function: split_on
...
When an exchange configuration authors its own `standardization`, that standardization is treated as authoritative: if it contradicts the linkage terms -- a transformation `output` naming no declared linkage field, or a `steps` entry naming an unknown `function` -- it fails closed (the CLI exits 64) with a message naming the offending output or function, rather than warning and proceeding past the contradiction. A direct `exchange`, and any run that mints no invitation, is refused during data preparation, before any credential, terms, or data are sent. On `exchange` that preparation precedes every connection the run makes, the first-use host-key probe of an unpinned SFTP configuration included; on a zero-setup run that probe is the one connection preceding the refusal, and it presents no credential (see [CHANNEL_SECURITY.md](spec/CHANNEL_SECURITY.md#sftp-host-key-verification)). A config-source `alcove invite` is refused at mint time, before the token is disclosed, so an inconsistent configuration never yields an invitation the same config's later `exchange` would reject. A configuration that authors no `standardization` reconstructs the default per-type cleaning from its metadata and terms, so it is unaffected by this check.

A `split_on` step fails closed under a combination that matches one value per record, whether it sits in a `standardization` or in a linkage key's element transform, and that combination is refused at every point the terms it reads are known. A count-only exchange is decided by one party's own terms, so it is refused at the same points as above -- when the terms are authored or minted into an invitation, and at data preparation -- and again once both parties' terms are agreed, rather than run with the narrower matching it would deliver (see [Fan-out (multi-value fields)](#fan-out-multi-value-fields)). The web app authors no `split_on` at all: its cleaning-step menu does not offer the function, and a terms document that declares one -- in a cleaning step or a linkage key's element transform -- blocks generation. A fan-out is therefore authored in a configuration file, as the example above does, and run with `alcove exchange` or minted with `alcove invite`.
...
A step declares at most 16 parameters, which is also how many the invitation's consent summary states for a step, so what a reader sees is what the run applies rather than a count standing for the rest. The summary names each parameter in the snake_case spelling the document schema uses -- `include_original`, not the camelCase form validation runs on -- and a refusal of that parameter names it the same way. A key the document writes in camelCase is shown in that snake_case spelling too, and a free-form key outside the lowercase-word convention gains an underscore before each capital: `keyMaterial` shows as `key_material`. Further step shapes are refused where a document is read, each for that same reason: a private key block in a parameter value, in a parameter name, or in the step's function name, each of which the summary shows as a redaction marker in place of the text the step declares, and a `null_if` step declaring both `value` and `values`, of which the run applies only the list. The party name a document states (`identity`) is refused for a private key block too, for the same reason: the summary would show the marker where a reader expects the partner naming itself. Every refusal fires on a partner's invitation and on your own configuration alike; see [CHANNEL_SECURITY.md](spec/CHANNEL_SECURITY.md#transform-parameter-declared-types).
...
The `split_on` function produces `set<string>` instead of a single `string`. When a transformation ends with a set, the field holds multiple candidate values. Each value generates a separate PSI entry for the row, but all entries retain the original row identifier so that a match resolves back to the source row.
...
**Either linkage strategy**: fan-out runs under [`cascade`](#linkage_termslinkage_strategy) and under `single-pass`, and both produce the same table on the same rows. What differs is the disclosure each strategy already carries: a single-pass receiver sees the sender's full per-key value structure, while a cascade round shows each party how the other's matched values in that round group into records. A key whose element splits costs 20 toward the single-pass dataset ceiling in place of 1, so a template with one such key holds proportionally fewer records; a fan-out you author in your own `standardization` instead multiplies the record count you declare, which costs the same and is what keeps it off the wire; the arithmetic is in [PROTOCOL.md](spec/PROTOCOL.md#the-width-bound-a-per-key-candidate-cap-the-terms-declare).

One combination refuses a fan-out rather than matching on one candidate, at every point the terms it reads are known: the count-only algorithm [`psi-c`](#linkage_termsalgorithm), whose round counts matched values and would report more links than the exchange stands for. It is decided by one party's own terms, so it is refused when they are authored or minted into an invitation, at data preparation, and again once both parties' terms are agreed. `deduplicate: true` on both parties does not refuse a fan-out: either [linkage strategy](#linkage_termslinkage_strategy) matches that pair over the two parties' candidates ([`linkage_terms.deduplicate`](#linkage_termsdeduplicate)).

**What the accepting party is told**: an invitation whose linkage keys split a value marks each such key element as matching on several values, and states the consequences the acceptor consents to -- matching per candidate, the record leaving the later keys once any candidate matches, and the candidate grouping the exchange's resolver is handed. Under a combination that refuses one, that same invitation states the refusal instead. Where an invitation's linkage strategy pairs no both-sided match, an acceptance screen that lets you set your own `deduplicate` states beside that control that setting it refuses the exchange, so you meet the refusal before accepting rather than at the accept; neither shipped strategy is in that position. A fan-out authored in your own `standardization` is not on that surface at all: a standardization is per-party and local, and no invitation holds one.

**Match resolution for fan-out**: when several PSI entries derived from the same row match in one linkage key's round, the parties do NOT communicate to resolve it. Both derive the same record-level pairing locally from what the exchange already holds: each round's matches are lifted to record pairs, a deterministic rule accepts at most one pair per record, and every record whose candidates matched at all leaves the candidate set for later keys -- so an individual matches once or not at all, and one whose evidence was contradictory ends unmatched rather than matching on a less precise key. The rule, its ordering, and the disclosure fan-out adds are specified in [PROTOCOL.md](spec/PROTOCOL.md#fan-out-matching-multi-value-key-candidates).
...
**Each parameter has a type, and a wrong one is refused when the document is read.** A parameter the table below describes as a string is text and must be quoted where YAML would otherwise read it as something else -- an unquoted `0`, `007`, `true`, or `null` is a number, a boolean, or a null, not the fill character, phone prefix, or literal you meant. A parameter declared as a type its function does not read is refused where the document is decoded, in a configuration file and in an invitation alike, naming the parameter and the type it got; the exchange does not start and no value is quietly substituted for it. This holds for the numeric and boolean parameters in the same way: `substring`'s `start` and `length` and `pad_left`'s `length` are whole numbers, and `split_on`'s `include_original` is `true` or `false`. Leave a parameter out to take the default the table states -- that, and not a `null`, is how a step declares it has no value for one. Two parameters the table marks required are the exception: `pattern` (`replace_regex`, `extract_regex`, `filter_regex`) and `delimiter` (`split_on`) have no default, so a step declaring neither is refused where the document is read, alongside a wrong type. The table's other required parameters -- `substring`'s `start` and `length` and `pad_left`'s `length` -- are not refused at that read; they are caught at the accept-boundary compile walk and again at the run.
...
A step the function cannot be built from at all is refused where the terms are minted, for a related reason: a pipeline is built once, before the first row is read, so a `pad_left` with no `length` or a fill longer than one character, a `phonetic` naming an algorithm this build does not implement, or a `function` it does not recognize stops the exchange before it matches anything -- on both parties, and only once the invitation has been accepted and set up against. Both front ends check at the mint, before the token is disclosed: a config-source `alcove invite`, as it is checked for a standardization that contradicts the terms, and the browser editor when Generate creates the invitation. Either refusal names the offending step's function, where the name is one this build recognizes, and the two remedies: correct that step's parameters, or remove the step. The browser states it at whichever click minted -- creating the invitation, or saving the exchange file a command-line partner runs -- rather than as a failure to retry, which no retry of the same document clears. In the browser editor an authored cleaning step is refused earlier still, at the step editor, which holds Generate shut until the step is corrected or removed: a malformed parameter is marked on the input that has to change, and a step naming a function the build does not recognize is marked on the step itself, since its parameters cannot be edited there and removing it is the only remedy. A linkage-key element transform takes the second of those earlier refusals and not the first: an unrecognized function holds Generate shut there too, while a parameter is judged only by what core can build from it, since an imported document's parameters are the partner's and the editor types them more strictly than the exchange runs them. Separately, a document declaring more transform steps than the mint checks -- counted across every cleaning step and every linkage-key element transform -- is refused at those same clicks with the count and the limit stated; the limit is far above what any authored document declares.
...
> **Either linkage strategy:** matching on the values `split_on` produces runs under [`cascade`](#linkage_termslinkage_strategy) and under `single-pass`. A count-only exchange refuses one before the exchange runs. A both-sided `deduplicate` runs under either strategy and groups the records the values reach. See [Fan-out (multi-value fields)](#fan-out-multi-value-fields).
...
- **Command line** -- whether an `alcove` command, flag, or prompt sets the value. Anything the command line does not set, you set by editing `alcove.yaml`.
- **Console** -- what opening an `alcove.yaml` from the working folder does with the value, and whether a console control changes it afterwards.
- **Web application** -- what importing an `alcove.yaml` as a stored exchange does with the value, and whether that exchange's settings change it afterwards.
