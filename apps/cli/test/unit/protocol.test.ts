...
test("warns on a close failure when the open completes after cleanup began", async () => {
  // SIGINT arrives while conn.open() is still awaited, so cleanup starts with
  // the run not yet marked opened and the open resolves while cleanup is
  ...
  const realOpen = FileSyncConnection.prototype.open;
  const openSpy = vi
    .spyOn(FileSyncConnection.prototype, "open")
    .mockImplementation(async function (
      this: FileSyncConnection,
      config: Parameters<FileSyncConnection["open"]>[0],
    ) {
      await realOpen.call(this, config);
      openReached = true;
      await openHeld;
    });
  const closeSpy = vi
    ...
    .mockImplementation(async () => {
      releaseOpen();
      // Yield past the released open()'s continuation so the run is marked
      // opened before this rejection reaches the cleanup catch.
  ...
  try {
    await vi.waitFor(() => expect(openReached).toBe(true), { timeout: 5_000 });
    process.emit("SIGINT");
    await vi.waitFor(
      () =>
        expect(
          mockState.warnings.some((m) =>
            m.includes("failed to close connection during cleanup"),
          ),
        ).toBe(true),
      { timeout: 5_000 },
    );
...
test("runProtocol invokes onAuthenticated after the rotated key is saved and before the exchange begins", async () => {
  ...
  const preparedA = minimalPreparedExchange({
    linkageTerms: getDefaultLinkageTerms("A"),
  });
  const preparedB = minimalPreparedExchange({
    linkageTerms: getDefaultLinkageTerms("B"),
  });
...
test("a successful run under --event-stream reports stage timing and counters", async () => {
  ...
  // zero times.
  const preparedWithRows = minimalPreparedExchange({ rowCount: 7 });
  vi.mocked(runExchange).mockImplementation((async (...args: unknown[]) => {
...
test(
  ...
  async () => {
    ...
    const preparedWithRows = minimalPreparedExchange({ rowCount: 4 });
    vi.mocked(runExchange).mockImplementation((async (...args: unknown[]) => {
