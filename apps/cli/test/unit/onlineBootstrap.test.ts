...
describe("looksLikeUrl", () => {
  test("supported transport schemes are URLs", () => {
  ...
  test("files, invitations, and other schemes are not URLs", () => {
...
describe("connectionFromURL", () => {
  test("sftp URL maps to an sftp connection", () => {
  ...
  test("file URL maps to a filedrop connection", () => {
  ...
  test("a file URL naming no local directory is a usage error", () => {
  ...
  test("a webrtc (ws) URL is a usage error", () => {
  ...
  test("a bare-host sftp URL leaves the path unset", () => {
  ...
  test("an sftp URL with no host is a usage error", () => {
  ...
  test("decodes a percent-encoded host", () => {
  ...
  test("and diffConnectionAgainstTarget agree on an encoded URL", () => {
    ...
    const { conflicts, warnings } = diffConnectionAgainstTarget(
      existing,
      target,
    );
...
describe("diffConnectionAgainstTarget", () => {
  test("a differing private-key passphrase warns name-only", () => {
  ...
  test("a differing keyboard-interactive setting warns", () => {
...
describe("connectionFromURL", () => {
  test("--outbound-path splits an sftp URL path into inbound/outbound", () => {
  ...
  test("--outbound-path splits a filedrop URL directory", () => {
...
describe("diffConnectionAgainstTarget", () => {
  test("a matching split pair is no conflict", () => {
  ...
  test("a differing split half conflicts on that field", () => {
    const target: RunnableConnectionConfig = {
      channel: "sftp",
      server: { host: "host", inboundPath: "/in", outboundPath: "/out" },
    };
  ...
  test("a shared config against a split target conflicts on both halves, naming the shared path", () => {
    ...
    expect(
      conflicts.every((c) =>
        c.existing.includes('single shared path "/mnt/in"'),
      ),
    ).toBe(true);
  });

  test("a split config against a shared target names the split locator", () => {
...
describe("warnServerOverridesIgnoredOffline", () => {
  test("one warning names every set flag", () => {
    const warnings: string[] = [];
    warnServerOverridesIgnoredOffline(
  ...
  test("stays silent when no override is set", () => {
    const warnings: string[] = [];
    warnServerOverridesIgnoredOffline({}, { warn: (m) => warnings.push(m) });
    expect(warnings).toEqual([]);
  });

  test("--no-server-keyboard-interactive (false) is not reported as ignored", () => {
...
describe("warnOptionsOverridesIgnoredOffline", () => {
  test("one warning names every set flag", () => {
    const warnings: string[] = [];
    warnOptionsOverridesIgnoredOffline(
  ...
  test("a negated boolean toggle (--no-*) does not warn", () => {
...
describe("connectionOverridesFrom", () => {
  test("maps pollingFrequencyMs to the pollIntervalMs override verbatim", () => {
  ...
  test("an absent --polling-frequency leaves pollIntervalMs unset", () => {
    const overrides = connectionOverridesFrom({});
    expect(overrides.options?.pollIntervalMs).toBeUndefined();
  });

  test("maps serverKeyboardInteractive into the server sub-group", () => {
    const overrides = connectionOverridesFrom({
...
describe("warnLowPollingFrequency", () => {
  test("warns below the 1s threshold on a file-sync channel", () => {
    const warnings: string[] = [];
  ...
  test("silent at exactly the 1s threshold", () => {
  ...
  test("silent above the threshold and when the flag is absent", () => {
    const warnings: string[] = [];
    warnLowPollingFrequency("sftp", 5000, { warn: (m) => warnings.push(m) });
    warnLowPollingFrequency("sftp", undefined, {
      warn: (m) => warnings.push(m),
    });
    expect(warnings).toEqual([]);
  });

  test("silent on a non-file-sync (or unresolved) channel even with a low value", () => {
...
describe("redactUrlCredentials", () => {
  test("strips an embedded password and username", () => {
  ...
  test("a credential-free URL is unchanged", () => {
...
describe("parseCommonBootstrapArgs", () => {
  ...
  test("a repeated number flag is a usage error naming the flag", () => {
  ...
  test("a repeated string flag is a usage error naming the flag", () => {
  ...
  test("--outbound-path is read as a string", () => {
  ...
  test("a repeated --outbound-path is a usage error", () => {
    expect(() =>
      parseCommonBootstrapArgs({
        _: [],
        $0: "alcove",
  ...
  test("human-readable timeouts parse to whole seconds", () => {
  ...
  test("a bare-integer timeout is rejected with the suffixed equivalent", () => {
    expect(() =>
      parseCommonBootstrapArgs({
        _: [],
        $0: "alcove",
  ...
  test("a malformed timeout is a flag-named usage error", () => {
    expect(() =>
      parseCommonBootstrapArgs({
        _: [],
        $0: "alcove",
  ...
  test("a connection-/peer-timeout above the 7d ceiling is rejected before any side effect", () => {
  ...
  test("a negative max-reconnect-attempts is a flag-named usage error", () => {
  ...
  test("a max-reconnect-attempts above the ceiling is rejected before any side effect", () => {
  ...
  test("a max-reconnect-attempts at the ceiling is accepted", () => {
  ...
  test("a non-numeric server-port is a flag-named usage error", () => {
...
describe("warnUnsupportedFileSyncFlags", () => {
  test("file-sync channels never warn", () => {
  ...
  test("a non-file-sync channel warns only for the flags set", () => {
  ...
  test("the filename toggles are reported too, by name only", () => {
...
describe("warnUnsupportedWebRTCServerFlags", () => {
  test("webrtc warns once per flag set", () => {
    const both = collectWarnings();
    warnUnsupportedWebRTCServerFlags(
      "webrtc",
  ...
  test("each remedy points at the connection the caller has", () => {
  ...
  test("every dropped credential flag is reported, by name only", () => {
  ...
  test("the credential line claims silence for the flags, not for the channel", () => {
  ...
  test("the file-sync channels never warn", () => {
...
describe("runOrExit", () => {
  ...
  test("shows a sanitized cause chain in the CLI failure output", async () => {
...
describe("connectionFromEndpoint", () => {
  test("no endpoint yields a marked sftp placeholder", () => {
  ...
  test("an sftp endpoint seeds the locator, marks credentials", () => {
  ...
  test("a filedrop endpoint seeds the shared path", () => {
    const endpoint: ConnectionEndpoint = {
      channel: "filedrop",
      path: "/mnt/share/drop",
    };
  ...
  test("a webrtc endpoint seeds the signaling locator", () => {
    const endpoint: ConnectionEndpoint = {
      channel: "webrtc",
  ...
  test("a webrtc endpoint's relay is kept as invitationRelay, not as this party's turn or stun", () => {
    const relay = {
  ...
  test("a split sftp endpoint mirror-swaps the inbound/outbound pair", () => {
  ...
  test("a split filedrop endpoint mirror-swaps the inbound/outbound pair", () => {
    const endpoint: ConnectionEndpoint = {
      channel: "filedrop",
...
describe("connectionFromEndpoint", () => {
  test.each(["sftp", "filedrop"] as const)(
    "the swapped %s split config validates against the connection schema",
    (channel) => {
...
describe("connectionFromEndpoint", () => {
  test("throws on a filedrop endpoint naming no directory", () => {
...
describe("applyEndpointSplitDirectories", () => {
  test("throws on an endpoint naming only one directory", () => {
  ...
  test("grafts a split sftp endpoint onto the URL connection, keeping host/credentials", () => {
  ...
  test("grafts a split filedrop endpoint onto a filedrop URL", () => {
    const urlConnection = connectionFromURL(
      platformFileUrl("/mnt/ignored"),
      {},
    );
    const endpoint: ConnectionEndpoint = {
      channel: "filedrop",
  ...
  test("preserves URL-derived options under the retain trio", () => {
    ...
    const { connection } = applyEndpointSplitDirectories(
      urlConnection,
      endpoint,
    );
  ...
  test("a non-split endpoint is a no-op", () => {
    const urlConnection = connectionFromURL(new URL("sftp://host/drop"), {});
    const endpoint: ConnectionEndpoint = {
      channel: "sftp",
      host: "inviter-host",
...
describe("applyEndpointSplitDirectories", () => {
  ...
  ] as const)("%s leaves the URL connection unchanged", (_label, endpoint) => {
    const urlConnection = connectionFromURL(new URL("sftp://host/drop"), {});
    const { connection, endpointDirectories } = applyEndpointSplitDirectories(
      urlConnection,
      endpoint,
...
describe("applyEndpointSplitDirectories", () => {
  test("a channel-mismatched endpoint places the roles per the URL's channel", () => {
  ...
  test("rejects a degenerate (relative-path) filedrop endpoint", () => {
...
describe("inviterConnectionFromURL", () => {
  test("a file-sync URL is built exactly as connectionFromURL builds it", () => {
    for (const url of [
  ...
  test("a wss URL maps to the coordination server's location", () => {
  ...
  test("a ws URL records the plaintext choice", () => {
  ...
  test("a bare-host webrtc URL leaves the port unset but names the mount point", () => {
  ...
  test("refuses pre-mint every shape the dial would refuse", () => {
  ...
  test("a plaintext webrtc URL raises no dial-time advisory at mint", () => {
  ...
  test("a webrtc URL holding anything past the location is refused", () => {
  ...
  test("the parser itself makes a host-less webrtc URL unreachable", () => {
  ...
  test("the parser never hands the builder an empty webrtc pathname", () => {
  ...
  test("the shared timeouts apply on webrtc, the file-sync options do not", () => {
  ...
  test("--outbound-path on a webrtc URL is refused, not dropped", () => {
...
describe("endpointFromConnection", () => {
  test("an sftp connection emits the host/port/path locator", () => {
  ...
  test("a bare-host sftp connection emits no path", () => {
  ...
  test("no credential rides along on the emitted endpoint", () => {
...
describe("endpointFromConnection", () => {
  test.each(["sftp", "filedrop"] as const)(
    "a split %s connection emits the inbound/outbound pair VERBATIM",
    (channel) => {
      ...
      const inbound =
        channel === "sftp"
          ? "/inviter-in"
          : platformAbsolutePath("/inviter-in");
      const outbound =
        channel === "sftp"
          ? "/inviter-out"
...
describe("endpointFromConnection", () => {
  test("-> connectionFromEndpoint round-trips a split pair mirror-swapped", () => {
  ...
  test("a webrtc connection emits the signaling locator", () => {
    const endpoint = endpointFromConnection(
  ...
  test("webrtc values the endpoint schema rejects are dropped", () => {
  ...
  test("-> connectionFromEndpoint round-trips a webrtc locator", () => {
  ...
  test("an over-long webrtc host is a clean usage error", () => {
    expect(() =>
      endpointFromConnection({
        channel: "webrtc",
  ...
  test("an over-long host is a clean usage error, not an opaque encode failure", () => {
    ...
    expect(() => endpointFromConnection(connection)).toThrow(
      /host is too long/,
    );
  });

  test("an over-long path is a clean usage error", () => {
    const connection = connectionFromURL(
  ...
  test("an over-long split outbound_path is a clean usage error", () => {
  ...
  test("a host at the length limit is accepted", () => {
...
describe("generateSharedSecret", () => {
  test("matches the shared secret format and is non-deterministic", () => {
...
describe("buildDataSpec", () => {
  test("infers linkage terms, metadata, and standardization from input (invite)", () => {
  ...
  test("without input rows, the spec is just the supplied terms (accept)", () => {
    const dataSpec = buildDataSpec({
      identity: "Agency B",
      rows: ROWS,
    });
  ...
  test("neither terms nor input rows is refused rather than yielding an empty spec", () => {
  ...
  test("supplied terms plus input infer metadata and standardization (accept)", () => {
...
describe("runOnlineBootstrap", () => {
  test("saves the first rotated key with an exclusive create", async () => {
  ...
  test("writes the config from the hook even when the exchange then fails", async () => {
  ...
  test("does not write the config when the handshake fails", async () => {
  ...
  test("marks the path in the refusal to overwrite a config that appeared late", async () => {
  ...
  test("names both written paths as the operator typed them when the exchange then fails", async () => {
  ...
  test("passes a webrtc connection through to the exchange and the saved config", async () => {
  ...
  test("spends a run-only peer budget on the run and writes none of it", async () => {
  ...
  test("leaves a connection's own peer budget on both the run and the config", async () => {
  ...
  test("returns the config-write error when the hook fails but the exchange succeeds", async () => {
  ...
  test("reports no config-write error on a clean run", async () => {
...
describe("runOnlineBootstrap", () => {
  test("hands runProtocol the emitter it opened itself, not the flag", async () => {
  ...
  test("writes no declaration for a party that accepted none", async () => {
...
describe("runOnlineBootstrap", () => {
  test("persists an @path credential as the reference while connecting with the resolved value", async () => {
  ...
  test("refuses a credential @path naming a missing file before the host-key step", async () => {
  ...
  test("refuses a first round too large for one message file before the host-key step", async () => {
  ...
  test("dials the connection the host-key step pinned", async () => {
  ...
  test("notes the config is on disk when the exchange fails after the config was written", async () => {
  ...
  test("does not log a config-on-disk note when the handshake fails", async () => {
  ...
  test("with reuseExistingConfig does not log a recovery note when the handshake fails before the key is saved", async () => {
  ...
  test("with reuseExistingConfig logs the recovery note when the exchange fails after the handshake", async () => {
...
describe("logOnlineBootstrapOutcome", () => {
  ...
  test("a config-write failure logs at error level and does not claim the config was saved", () => {
    const log = {
      info: vi.fn(),
      warn: vi.fn(),
      error: vi.fn(),
  ...
  test("a reused config reports the existing config and the rotated key", () => {
    const log = {
      info: vi.fn(),
      warn: vi.fn(),
      error: vi.fn(),
...
describe("runOnlineBootstrap", () => {
  test("with reuseExistingConfig keeps the existing config and reports no write error", async () => {
  ...
  test("re-gates the config write: a config appearing after the check is not silently overwritten", async () => {
  ...
  test("refuses a config that appears after its re-gate, leaving it as it was", async () => {
    vi.mocked(runProtocol).mockImplementation((async (
      ...callArgs: unknown[]
    ) => {
      const onAuthenticated = onAuthenticatedArg(callArgs);
...
describe("diffConnectionAgainstTarget", () => {
  test("an agreeing sftp config has no conflicts or warnings", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
      server: {
        host: "host",
  ...
  test("a host mismatch is a conflict (which drop)", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("host comparison is case-insensitive (same endpoint)", () => {
  ...
  test("an sftp path mismatch is a conflict (which drop)", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("a trailing-slash-only path difference is not a conflict", () => {
  ...
  test("a path the target omits is not flagged", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
      server: { host: "host", path: "/drop" },
    };
  ...
  test("a differing port warns (how you reach), not conflicts", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("a target port equal to the config is silent", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
      server: { host: "host", port: 2222 },
    };
  ...
  test("the default port 22 against an unset config is silent", () => {
  ...
  test("a non-default port against an unset config warns", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
      server: { host: "host" },
    };
  ...
  test("credentials the target omits are not flagged", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("differing credentials warn without echoing the value", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("a differing private key warns without echoing it", () => {
    const existing: ConnectionConfig = {
      channel: "sftp",
  ...
  test("a channel mismatch warns and compares nothing else (file-sync)", () => {
  ...
  test("a filedrop path mismatch is a conflict", () => {
    const existing: ConnectionConfig = {
      channel: "filedrop",
      path: "/mnt/other",
    };
  ...
  test("a filedrop trailing-slash-only difference is not a conflict", () => {
    const existing: ConnectionConfig = {
      channel: "filedrop",
      path: "/mnt/share",
    };
  ...
  test("a filedrop path differing only by multiple trailing slashes is not a conflict", () => {
  ...
  test("a filedrop path differing only by backslashes is not a conflict", () => {
...
describe("loadInputRows", () => {
  test("a CSV piped via `-` yields the same rows as the equivalent file (invite path)", async () => {
  ...
  test("a dataset with no data rows refuses, from a file or stdin", async () => {
    ...
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), "alcove-loadrows-empty-"),
    );
    try {
  ...
  test("a row-level parse fault refuses before any exchange work", async () => {
    ...
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), "alcove-loadrows-fault-"),
    );
    try {
      const file = path.join(dir, "in.csv");
      fs.writeFileSync(
  ...
  test("`-` is rejected as a usage error when stdin is disallowed (accept path)", async () => {
  ...
  test("`-` at an interactive terminal is rejected (invite path inherits the TTY guard)", async () => {
  ...
  test("a tab inside a header name is removed and reported too", async () => {
  ...
  test("a bidi-stripped header is reported by position, never by name", async () => {
  ...
  test("a header that collides after the strip is warned by position, and the parser numbers the later column", async () => {
    ...
    // numbering instead.
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), "alcove-loadrows-clash-"),
    );
    const logged: Array<string> = [];
    const previousSink = getDiagnosticSink();
    const log = getLogger("input");
...
describe("inferDateInputFormatFromSource", () => {
  test("the init path infers the same metadata, fields, standardization, and dob format as a full read", async () => {
  ...
  test("reads a bounded DOB sample, so a file far larger than the scan cap still yields the header and format", async () => {
  ...
  test("a file with no dob column yields no format and infers the same terms as a full read", async () => {
    ...
    try {
      ...
      expect(inferred.columns).toEqual([
        "first_name",
        "last_name",
        "member_id",
      ]);
  ...
  test("a header larger than the read buffer is read whole, matching the full read", async () => {
  ...
  test("a `-` CSV from stdin infers the same as the file", async () => {
  ...
  test("a no-newline input fails fast rather than buffering the span", async () => {
...
describe("parseLinkageStrategyFlag", () => {
  test("absent selection is undefined (terms keep the cascade default)", () => {
    expect(
  ...
  test("an unknown value is a usage error (exit 64 via runOrExit)", () => {
  ...
  test("a repeated flag is rejected before the enum check", () => {
...
describe("buildDataSpec", () => {
  test("--linkage-strategy single-pass authors single-pass terms", () => {
    const dataSpec = buildDataSpec({
      identity: "tester",
      rows: STRATEGY_ROWS,
  ...
  test("omitting the selection authors cascade (unchanged from today)", () => {
    const dataSpec = buildDataSpec({
      identity: "tester",
      rows: STRATEGY_ROWS,
    });
  ...
  test("a supplied terms object (accept's path) ignores the selection", () => {
...
describe("singlePassDisclosureNotice", () => {
  test("names the disclosure tradeoff and the operator-facing doc", () => {
